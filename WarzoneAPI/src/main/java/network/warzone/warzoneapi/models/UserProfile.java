...
public class UserProfile {
  ...
  public static final int XP_PER_KILL = 2, XP_PER_WIN = 10, XP_PER_LOSS =
    5, XP_PER_WOOL_BREAK = 3;
  ...
  public Punishment getLatestMute() {
    if (punishments != null && !punishments.isEmpty()) {
      for (Punishment punishment : getPunishments()) {
        if (
          "MUTE".equalsIgnoreCase(punishment.getType()) && punishment.isActive()
        ) return punishment;
      }
    }
    return null;
  ...
  public Punishment getLatestBan() {
    if (punishments != null && !punishments.isEmpty()) {
      for (Punishment punishment : getPunishments()) {
        if (
          "BAN".equalsIgnoreCase(punishment.getType()) && punishment.isActive()
        ) return punishment;
      }
    }
    return null;
  ...
  public String getPrefix() {
    if (!ranksLoaded.isEmpty()) {
      ...
      return highest.getPrefix() != null && !highest.getPrefix().isEmpty()
        ? highest.getPrefix()
        : null;
    } else return null;
  }
