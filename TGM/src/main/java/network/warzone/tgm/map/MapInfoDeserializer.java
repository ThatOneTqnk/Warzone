...
public class MapInfoDeserializer implements JsonDeserializer<MapInfo> {
  ...
  public MapInfo deserialize(
    ...
    String objective = json.has("objective")
    ...
    for (JsonElement authorJson : json.getAsJsonArray("authors")) {
      if (authorJson.isJsonPrimitive()) {
        ...
        if (TGM.get().getConfig().getBoolean("map.get-names")) Bukkit
          .getScheduler()
          .runTaskAsynchronously(
            ...
            () -> {
              if (author != null && author.getUuid() != null) {
                try {
                  MojangProfile profile = ProfileCache
                    .getInstance()
                    .get(author.getUuid());
                  if (profile == null) {
                    profile =
                      TGM
                        .get()
                        .getTeamClient()
                        .getMojangProfile(author.getUuid());
                  ...
                  TGM
                    ...
                    .getLogger()
                    .warning(
                      "Could not retrieve current name for " +
    ...
    for (JsonElement teamElement : json.getAsJsonArray("teams")) {
      ...
      ChatColor teamColor = ChatColor.valueOf(
        teamJson.get("color").getAsString().toUpperCase().replace(" ", "_")
      );
      GameMode teamGamemode = teamJson.has("gamemode")
        ? GameMode.valueOf(
          Strings.getTechnicalName(teamJson.get("gamemode").getAsString())
        )
        : GameMode.SURVIVAL;
      int teamMax = teamJson.get("max").getAsInt();
      int teamMin = teamJson.get("min").getAsInt();
      boolean friendlyFire =
        teamJson.has("friendlyFire") &&
        teamJson.get("friendlyFire").getAsBoolean();
      parsedTeams.add(
        new ParsedTeam(
          teamId,
          teamName,
