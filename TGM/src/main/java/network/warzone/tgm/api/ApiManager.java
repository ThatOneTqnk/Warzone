...
public class ApiManager implements Listener {
  ...
  public ApiManager() {
    ...
    if (TGM.get().getTeamClient() instanceof HttpClient) Bukkit
      .getScheduler()
      .runTaskTimerAsynchronously(
        ...
        () -> {
          ...
          for (PlayerContext playerContext : TGM
            .get()
            .getPlayerManager()
            .getPlayers()) {
            try {
  ...
  public void onMatchResult(MatchResultEvent event) {
    ...
    try {
      ...
      if (event.getWinningTeam() != null) {
        for (PlayerContext playerContext : event
          .getWinningTeam()
          .getMembers()) {
          ...
          Bukkit
            ...
            .callEvent(
              new PlayerXPEvent(
                playerContext,
                UserProfile.XP_PER_WIN,
                playerContext.getUserProfile().getXP() - UserProfile.XP_PER_WIN,
      ...
      for (MatchTeam matchTeam : teamManagerModule.getTeams()) {
        ...
        for (PlayerContext playerContext : matchTeam.getMembers()) {
          teamMappings.add(
            new TeamMapping(
              matchTeam.getId(),
              playerContext.getUserProfile().getId().toString()
            )
          );
        }
      }
    ...
    try {
      ...
      Bukkit
        ...
        .runTaskAsynchronously(
          TGM.get(),
          () -> {
            MapLoadResponse mapLoadResponse = TGM
              .get()
              .getTeamClient()
              .loadmap(
                new Map(
            ...
            Bukkit
              .getLogger()
              .info(
                "Received load map response. Id: " +
                mapLoadResponse.getMap() +
                " [" +
                mapLoadResponse.isInserted() +
                "]"
              );
            matchInProgress =
              TGM
                .get()
                .getTeamClient()
                .loadMatch(new MatchLoadRequest(mapLoadResponse.getMap()));
            Bukkit
              .getLogger()
              .info(
                "Match successfully loaded [" + matchInProgress.getMap() + "]"
              );
          }
        );
  ...
  public void onKill(TGMPlayerDeathEvent event) {
    ...
    try {
      ...
      PlayerContext killed = TGM
        .get()
        .getPlayerManager()
        .getPlayerContext(deathInfo.player);

      killed.getUserProfile().addDeath();

      String playerItem = deathInfo.player
        .getInventory()
        .getItemInMainHand()
        .getType()
        .toString();
      String killerItem = deathInfo.item == null
        ? ""
        : deathInfo.item.getType().toString();
      ...
      if (deathInfo.killer != null) {
        PlayerContext context = TGM
          .get()
          .getPlayerManager()
          .getPlayerContext(deathInfo.killer);
        ...
        Bukkit
          ...
          .callEvent(
            new PlayerXPEvent(
              context,
              UserProfile.XP_PER_KILL,
              context.getUserProfile().getXP() - UserProfile.XP_PER_KILL,
              context.getUserProfile().getXP()
            )
          );
      ...
      Death death = new Death(
        killed.getUserProfile().getId().toString(),
        killerId,
        playerItem,
        killerItem,
        matchInProgress.getMap(),
  ...
  public boolean isStatsDisabled() {
    return (
      ...
      TGM.get().getModule(StatsModule.class).isStatsDisabled()
    );
  }
}
