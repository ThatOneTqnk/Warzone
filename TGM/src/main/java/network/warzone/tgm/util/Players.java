...
public class Players {
  ...
  public static void reset(Player player, boolean heal, boolean keepInv) {
    ...
    if (!keepInv) {
      for (Attribute attribute : Attribute.values()) {
        ...
        for (AttributeModifier modifier : player
    ...
    player.addPotionEffect(
      new PotionEffect(PotionEffectType.FIRE_RESISTANCE, 1, 0)
    ); // Weird lava bug
  ...
  public static Optional<PlayerContext> getNearestPlayer(
      ...
      .min(
        (p1, p2) -> {
          ...
          return Integer.compare(
            (int) loc.distance(loc1),
            (int) loc.distance(loc2)
          );
        }
      );
  }
