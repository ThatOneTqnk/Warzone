...
public class TimeUnitPair {
  ...
  public String toString() {
    if (toString == null) {
      if (this.unit == ChronoUnit.FOREVER || toMilliseconds() == -1) toString =
        "permanent"; else if (value == 1) {
        if (this.unit == ChronoUnit.MILLENNIA) toString =
          value + " millennium"; else if (
          this.unit == ChronoUnit.CENTURIES
        ) toString = value + " century"; else if (
          this.unit.name().toLowerCase().endsWith("s")
        ) {
          toString =
            value +
            " " +
            this.unit.name()
              .substring(0, this.unit.name().length() - 1)
              .toLowerCase()
              .replace("_", " ");
        } else toString =
          value + " " + this.unit.name().toLowerCase().replace("_", " ");
      } else toString =
        value + " " + this.unit.name().toLowerCase().replace("_", " ");
    }
    return toString;
