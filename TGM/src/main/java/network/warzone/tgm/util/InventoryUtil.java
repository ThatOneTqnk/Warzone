...
public class InventoryUtil {
  ...
  public static Inventory clone(Inventory i, String title) {
    Inventory inventory;
    if (title == null) {
      if (i.getType() == InventoryType.CHEST) inventory =
        Bukkit.createInventory(i.getHolder(), i.getSize()); else inventory =
        Bukkit.createInventory(i.getHolder(), i.getType());
    } else {
      if (i.getType() == InventoryType.CHEST) inventory =
        ...
        Bukkit.createInventory(i.getHolder(), i.getType(), title);
    }
  ...
  public static void setPlayerInventoryContents(
    Player player,
    Inventory inventory
  ) {
    ...
    for (int i = 0; i < 36; i++) {
      ...
      if (i < 9) inventory.setItem(36 + i, itemStack); else inventory.setItem(
        i,
        itemStack
      );
    }
  ...
  private static ItemStack getPotionsItem(
    Collection<PotionEffect> potionEffects
  ) {
    if (potionEffects == null || potionEffects.isEmpty()) {
      ItemStack itemStack = ItemFactory.createItem(
        Material.GLASS_BOTTLE,
        ChatColor.AQUA + "Active potions effects"
      );
