...
public class FireworkUtil {
  ...
  public static @Nonnull Firework spawnFirework(
    ...
    FireworkMeta meta = (FireworkMeta) Bukkit
      .getItemFactory()
      .getItemMeta(Material.FIREWORK_ROCKET);
    meta.setPower(power > 0 ? (power - 1) : power);
    meta.addEffect(effect);

    Firework firework = (Firework) location
      .getWorld()
      .spawnEntity(location.add(0.5, 0.0, 0.5), EntityType.FIREWORK);
    firework.setFireworkMeta(meta);
    if (power == 0) Bukkit
      .getScheduler()
      .runTaskLater(TGM.get(), firework::detonate, 1L);
