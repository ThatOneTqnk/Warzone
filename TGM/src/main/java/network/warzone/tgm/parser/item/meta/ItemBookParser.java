...
public class ItemBookParser implements ItemMetaParser {
  ...
  static {
    try {
      ...
      classCraftMetaBook =
        Class.forName(
          "org.bukkit.craftbukkit.v1_16_R3.inventory.CraftMetaBook"
        );
      fieldPages = classCraftMetaBook.getDeclaredField("pages");
    } catch (
      ClassNotFoundException | NoSuchFieldException | NoSuchMethodException e
    ) {
      e.printStackTrace();
    }
  }
  ...
  public void parse(ItemStack itemStack, ItemMeta meta, JsonObject object) {
    ...
    bookMeta.setTitle(
      object.has("title")
        ? ColorConverter.filterString(object.get("title").getAsString())
        : "Empty Book"
    );
    bookMeta.setAuthor(
      object.has("author")
        ? ColorConverter.filterString(object.get("author").getAsString())
        : "Mojang"
    );
    bookMeta.setGeneration(
      object.has("generation")
        ? BookMeta.Generation.valueOf(
          Strings.getTechnicalName(object.get("generation").getAsString())
        )
      ...
      try {
        ...
        object
          ...
          .forEach(
            jsonElement -> {
              try {
                String page = (String) methodA.invoke(
                  ColorConverter.filterString(jsonElement.getAsString())
                );
                pages.add(page);
              } catch (Exception e) {
                e.printStackTrace();
