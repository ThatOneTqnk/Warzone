...
public class BroadcastManager {
  ...
  private File configFile = new File(
    TGM.get().getDataFolder().getAbsolutePath() + "/broadcasts.yml"
  );

  private File broadcastsFile = new File(
    TGM.get().getDataFolder().getAbsolutePath() + "/broadcasts.json"
  );
  ...
  @Getter
  private int interval = 60 * 20; // ticks (20 ticks = 1 second)
        ...
        .runTaskAsynchronously(
          ...
          () -> {
            try {
              ...
              System.out.println(
                "Could not retrieve broadcasts. Using local file."
              );
              e.printStackTrace();
              readFromFile();
              startAutoBroadcast(false);
  ...
  private void readFromURL() throws UnirestException {
    HttpResponse<JsonNode> broadcasts = Unirest
      .get(url)
      .asObject(JsonNode.class);
    Gson gson = new Gson();
    if (
      this.broadcasts != null && !this.broadcasts.isEmpty()
    ) this.broadcasts.clear();
    this.broadcasts =
      new ArrayList(
        Arrays.asList(
          gson.fromJson(broadcasts.getBody().toString(), Broadcast[].class)
        )
      );
  }

  private void readFromFile() {
    if (!this.broadcastsFile.exists()) TGM
      .get()
      .saveResource("broadcasts.json", true);
    try {
      Gson gson = new Gson();
      ...
      this.broadcasts =
        new ArrayList(
          gson.fromJson(reader, new TypeToken<List<Broadcast>>() {}.getType())
        );
  ...
  private boolean createConfig() {
    configFile =
      new File(TGM.get().getDataFolder().getAbsolutePath() + "/broadcasts.yml");
  ...
  private void loadConfig() {
    ...
    if (
      this.config.contains("autobroadcast") &&
      this.config.isBoolean("autobroadcast")
    ) this.autoBroadcast = this.config.getBoolean("autobroadcast");
    if (
      this.config.contains("queue") && this.config.isList("queue")
    ) this.queue = this.config.getStringList("queue");
    if (
      this.config.contains("interval") && this.config.isInt("interval")
    ) this.interval = this.config.getInt("interval");
    if (
      this.config.contains("url") &&
      this.config.isString("url") &&
      isURL(this.config.getString("url"))
    ) this.url = this.config.getString("url");
    if (
      this.config.contains("events") &&
      this.config.isConfigurationSection("events")
    ) {
      onEvents.clear();
      ConfigurationSection events =
        this.config.getConfigurationSection("events");
      for (String key : events.getKeys(false)) {
        if (events.isList(key)) addOnEvent(
          key,
          events.getStringList(key)
        ); else addOnEvent(
          key,
          Collections.singletonList(events.getString(key))
        );
      }
    }
  }
  ...
  public boolean broadcast(String id, String... args) {
    ...
    return (
      !broadcasts.isEmpty() &&
      (broadcast = getBroadcast(id)) != null &&
  ...
  public boolean broadcast(Player player, String id, String... args) {
    ...
    return (
      !broadcasts.isEmpty() &&
      (broadcast = getBroadcast(id)) != null &&
      broadcast(player, broadcast, args)
    );
  }
    ...
    if (permission != null) {
      if (permission.startsWith("!")) {
        for (Player player : Bukkit.getOnlinePlayers()) {
          if (
            !player.hasPermission(permission.substring(1, permission.length()))
          ) broadcastRaw(player, String.format(broadcast.getMessage(), args));
        }
      } else {
        for (Player player : Bukkit.getOnlinePlayers()) {
          if (player.hasPermission(permission)) broadcastRaw(
            player,
            String.format(broadcast.getMessage(), args)
          );
        }
      }
