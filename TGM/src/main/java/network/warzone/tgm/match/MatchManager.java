...
public class MatchManager {
  ...
  public void endMatch(MatchTeam winningTeam) {
    ...
    Bukkit
      .getPluginManager()
      .callEvent(new MatchResultEvent(match, winningTeam, losers));
  }
  ...
  public void cycleNextMatch() {
    ...
    try {
      FileUtils.copyDirectory(
        mapContainer.getSourceFolder(),
        new File("matches/" + matchUuid.toString())
      );
    } catch (IOException e) {
      e.printStackTrace();
    }
    ...
    if (oldMatch != null) {
      ...
      TGM
        ...
        .info(
          "Unloading match: " +
          oldMatch.getUuid().toString() +
          " (File: " +
          oldMatch.getWorld().getWorldFolder().getPath() +
          ")"
        );
      ...
      boolean save = TGM
        .get()
        .getConfig()
        .getBoolean("map.save-matches", false);
