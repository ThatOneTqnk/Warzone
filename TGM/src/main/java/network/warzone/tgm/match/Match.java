...
public class Match {
  ...
  public void load() {
    modules.addAll(
      matchManifest.allocateCoreModules(
        mapContainer.getMapInfo().getJsonObject()
      )
    );
  ...
  public List<MatchModule> getModules(ModuleLoadTime moduleLoadTime) {
    ...
    for (MatchModule matchModule : modules) {
      if (matchModule.getClass().isAnnotationPresent(ModuleData.class)) {
        if (
          matchModule.getClass().getAnnotation(ModuleData.class).load() ==
          moduleLoadTime
        ) {
