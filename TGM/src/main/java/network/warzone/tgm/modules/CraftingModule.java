...
public class CraftingModule extends MatchModule implements Listener {
  ...
  public void load(Match match) {
    ...
    if (c != null && c.isJsonObject()) {
      ...
      if (crafting.has("remove")) {
        if (crafting.get("remove").isJsonArray()) {
          ...
          crafting.get("remove").isJsonPrimitive() &&
          "*".equals(crafting.get("remove").getAsString())
        ) {
          removeAll = true;
        }
      }
    ...
    if (removeAll) Bukkit.clearRecipes(); else if (
      !this.removedRecipes.isEmpty()
    ) removeRecipes();
  }
  ...
  private static Recipe parseRecipe(JsonObject jsonObject) {
    ...
    NamespacedKey namespacedKey = KeyUtil.tgm(
      result.getType().name() + new Date().getTime()
    );
    switch (type) {
      case "shapeless":
      ...
      case "shaped":
        ...
        return shapedRecipe;
      case "furnace":
        RecipeChoice furnaceIngredient = parseRecipeIngredient(
          jsonObject.get("ingredient")
        );
        if (furnaceIngredient == null) return null;
        int furnaceExp = 0;
        int furnaceCookingTime = 200;
        if (jsonObject.has("experience")) furnaceExp =
          jsonObject.get("experience").getAsInt();
        if (jsonObject.has("cookingTime")) furnaceCookingTime =
          jsonObject.get("cookingTime").getAsInt();
      ...
      case "smoking":
        RecipeChoice smokingIngredient = parseRecipeIngredient(
          jsonObject.get("ingredient")
        );
        if (smokingIngredient == null) return null;
        int smokingExp = 0;
        int smokingCookingTime = 100;
        if (jsonObject.has("experience")) smokingExp =
          jsonObject.get("experience").getAsInt();
        if (jsonObject.has("cookingTime")) smokingCookingTime =
          jsonObject.get("cookingTime").getAsInt();
      ...
      case "blasting":
        RecipeChoice blastingIngredient = parseRecipeIngredient(
          jsonObject.get("ingredient")
        );
        if (blastingIngredient == null) return null;
        int blastingExp = 0;
        int blastingCookingTime = 100;
        if (jsonObject.has("experience")) blastingExp =
          jsonObject.get("experience").getAsInt();
        if (jsonObject.has("cookingTime")) blastingCookingTime =
          jsonObject.get("cookingTime").getAsInt();
      ...
      case "campfire":
        RecipeChoice campfireIngredient = parseRecipeIngredient(
          jsonObject.get("ingredient")
        );
        if (campfireIngredient == null) return null;
        int campfireExp = 0;
        int campfireCookingTime = 600;
        if (jsonObject.has("experience")) campfireExp =
          jsonObject.get("experience").getAsInt();
        if (jsonObject.has("cookingTime")) campfireCookingTime =
          jsonObject.get("cookingTime").getAsInt();
      ...
      case "stonecutting":
        RecipeChoice stonecuttingIngredient = parseRecipeIngredient(
          jsonObject.get("ingredient")
        );
