...
public class SpawnPointHandlerModule extends MatchModule implements Listener {
  ...
  public void spawnPlayer(
    ...
    if (teleport) {
      ...
      if (
        !matchTeam.isSpectator() && !gameType.equals(GameType.Infected)
    ...
    if (gameClassModule != null) {
      Bukkit
        ...
        .runTaskLater(
          ...
          () -> {
            if (
              gameClassModule.getCurrentClass(playerContext.getPlayer()) == null
            ) gameClassModule.setCurrentClass(
              playerContext.getPlayer(),
              gameClassModule.getDefaultClass()
            );
            if (matchTeam.isSpectator()) {
              ...
              if (reset) {
                ...
                GameClass gameClass = gameClassModule.getGameClass(
                  gameClassModule.getCurrentClass(playerContext.getPlayer())
                );
                if (gameClass != null) gameClass.apply(
                  playerContext.getPlayer(),
                  matchTeam.getColor()
                );
          ...
          1L
        ); // Delay by 1 tick to prevent missing armor points bug
    } else Bukkit
      .getScheduler()
      .runTaskLater(
        TGM.get(),
        () -> {
          ...
          if (reset) {
            matchTeam
              .getKits()
              .forEach(kit -> kit.apply(playerContext.getPlayer(), matchTeam));
            playerContext.getPlayer().updateInventory();
          }
          playerContext.getPlayer().setFireTicks(-20); // Weird lava bug
        },
