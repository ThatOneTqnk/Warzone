...
public class CountdownManagerModule extends MatchModule {
  ...
  public void load(Match match) {
    ...
    customCountdowns
      .values()
      .forEach(countdown -> match.getModules().add(countdown));
  }
  ...
  public CustomCountdown getCountdown(JsonElement jsonElement) {
    if (jsonElement.isJsonPrimitive()) {
      ...
      boolean visible =
        !countdownObj.has("visible") ||
        countdownObj.get("visible").getAsBoolean();
      boolean invert =
        countdownObj.has("invert") && countdownObj.get("invert").getAsBoolean();
      List<MatchTeam> teams = new ArrayList<>();
      List<String> onFinish = new ArrayList<>();
      if (countdownObj.has("color")) color =
        BarColor.valueOf(
          Strings.getTechnicalName(countdownObj.get("color").getAsString())
        );
      if (countdownObj.has("style")) style =
        BarStyle.valueOf(
          Strings.getTechnicalName(countdownObj.get("style").getAsString())
        );
      if (countdownObj.has("teams")) teams.addAll(
        teamManagerModule.getTeams(countdownObj.get("teams").getAsJsonArray())
      );
      ...
      CustomCountdown customCountdown = new CustomCountdown(
        time,
        title,
        color,
        style,
      ...
      if (countdownObj.has("id")) {
        this.customCountdowns.put(
            countdownObj.get("id").getAsString(),
            customCountdown
          );
      }
