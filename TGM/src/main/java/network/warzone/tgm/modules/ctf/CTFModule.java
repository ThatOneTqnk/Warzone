...
public class CTFModule extends MatchModule implements CTFControllerSubscriber {
  ...
  public void load(Match match) {
    ...
    CTFObjective objective = CTFObjective.valueOf(
      Strings.getTechnicalName(ctfJson.get("objective").getAsString())
    );
    ...
    if (objective == CTFObjective.TIME) {
      ...
      this.controller =
        new CTFAmountController(this, matchFlags, captureAmount);
    }
    ...
    for (JsonElement flagElem : ctfJson.get("flags").getAsJsonArray()) {
      this.matchFlags.add(
          MatchFlag.deserialize(flagElem.getAsJsonObject(), controller, world)
        );
    }

    // Don't allow flags to be dropped
    ItemRemoveModule itemRemoveModule = TGM
      .get()
      .getModule(ItemRemoveModule.class);
    ...
    if (objective == CTFObjective.AMOUNT) {
      RegionManagerModule regionManagerModule = TGM
        .get()
      ...
      for (JsonElement element : ctfJson.get("bases").getAsJsonArray()) {
        ...
        Region baseRegion = regionManagerModule.getRegion(
          match,
          baseObject.get("region")
        );
        MatchTeam matchTeam = teamManagerModule.getTeamById(
          baseObject.get("team").getAsString()
        );
