...
public class GameRuleModule extends MatchModule {
  ...
  public void load(Match match) {
    ...
    if (jsonObject.has("gamerules")) {
      for (Map.Entry<String, JsonElement> gameRuleEntry : jsonObject
        .getAsJsonObject("gamerules")
        .entrySet()) {
        ...
        if (gameRule.getType() == Boolean.class) {
          match
            .getWorld()
            .setGameRule(gameRule, gameRuleEntry.getValue().getAsBoolean());
        } else if (gameRule.getType() == Integer.class) {
          match
            .getWorld()
            .setGameRule(gameRule, gameRuleEntry.getValue().getAsInt());
        }
      }
    }
  }
