...
public class RespawnModule extends MatchModule implements Listener {
  ...
  @Getter
  @Setter
  private RespawnRule defaultRule = new RespawnRule(
    null,
    3000,
    false,
    true,
  ...
  public void load(Match match) {
    ...
    TeamManagerModule teamManagerModule = match.getModule(
      TeamManagerModule.class
    );
    ...
    if (mapInfo.has("respawn")) {
      ...
      if (respawnObject.has("rules")) {
        ...
        for (JsonElement element : rules) {
          ...
          if (rule.has("teams")) matchTeams =
            teamManagerModule.getTeams(rule.get("teams").getAsJsonArray());
          if (rule.has("delay")) delay = rule.get("delay").getAsInt();
          if (rule.has("freeze")) freeze = rule.get("freeze").getAsBoolean();
          if (rule.has("blindness")) blindness =
            rule.get("blindness").getAsBoolean();
          if (rule.has("confirm")) confirm = rule.get("confirm").getAsBoolean();
          this.respawnRules.add(
              new RespawnRule(matchTeams, delay, freeze, blindness, confirm)
            );
        }
      }
    }
  ...
  public void onTeamSwitch(TeamChangeEvent event) {
    ...
    if (
      !event.isForced() && isDead(event.getPlayerContext().getPlayer())
    ) event.setCancelled(true); else remove(
      event.getPlayerContext().getPlayer()
    );
  }

  @EventHandler
  public void onMatchResult(MatchResultEvent event) {
    new ArrayList<>(this.respawning.keySet())
    .forEach(p -> this.respawn(p, true));
  }

  @EventHandler
  public void onTeleport(PlayerTeleportEvent event) {
    if (
      isDead(event.getPlayer()) &&
            ...
            for (Map.Entry<Player, RespawnGoal> respawnEntry : this.respawning.entrySet()) {
              try {
                ...
                if (
                  getTimeLeft(goal) > 0 ||
                  !shouldRespawn ||
                  (goal.getRule().isConfirm() && !goal.isConfirmed())
                ) {
                  continue;
                }
  ...
  private void sendTitle(
    ...
    if (!shouldRespawn) {
      if (timeLeft >= -40 && timeLeft <= 0) player.sendTitle(
        format("&c&lYOU DIED"),
        "",
        0,
        20,
    ...
    player.sendTitle(
      format("&c&lYOU DIED"),
      format(
        "&7Respawning in &e" + String.format("%.1f", timeLeft / 1000.0) + 's'
      ),
      0,
      20,
      0
