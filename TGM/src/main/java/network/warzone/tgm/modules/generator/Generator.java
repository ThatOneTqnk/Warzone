...
public class Generator {
  ...
  public Generator(
    ...
    int range,
    int interval,
    GeneratorHologram generatorHologram,
    GeneratorUpgrader generatorUpgrader
  ) {
    this.id = id;
    this.item = item;
    ...
    if (this.generatorUpgrader != null) this.generatorUpgrader.setHostGenerator(
        this
      );
  ...
  public void enable() {
    if (this.generatorHologram != null) {
      ...
      generatorHologram.displayContent(
        item.getType(),
        currentInterval,
        generatorUpgrader.getGeneratorLevel()
      );
    }
    ...
    if (generatorHologram.getTimeDisplay() == TimeDisplay.TICKS) {
      ...
      generatorTimeHandle =
        () -> {
          ...
          if (elapsedTime % 1200 == 0) generatorHologram.displayContent(
            item.getType(),
            currentInterval,
            generatorUpgrader.getGeneratorLevel()
          );
      ...
      generatorTimeHandle =
        () -> {
          int elapsedTime = interval - currentInterval;
          if (elapsedTime % 20 == 0) generatorHologram.displayContent(
            item.getType(),
            currentInterval,
            generatorUpgrader.getGeneratorLevel()
          );
  ...
  private void perform() {
    if (
      range >= 0 &&
      location.getWorld().getNearbyPlayers(location, range).size() == 0
    ) return;
    if (limit > 0) {
      int nearbySimilarItems = location
        .getWorld()
        .getNearbyEntitiesByType(
          Item.class,
          location,
          2,
          theEntity -> theEntity.getItemStack().getType() == item.getType()
        )
        .stream()
        .mapToInt(itemEntity -> itemEntity.getItemStack().getAmount())
        .sum();
