...
public class ItemRemoveModule extends MatchModule implements Listener {
  ...
  public void addAll(Collection<Material> materials) {
    removed.addAll(
      materials.stream().map(ItemRemoveInfo::new).collect(Collectors.toSet())
    );
  }
  ...
  public void load(Match match) {
    if (
      match.getMapContainer().getMapInfo().getJsonObject().has("itemremove")
    ) {
      if (
        match
          .getMapContainer()
      ...
      for (JsonElement itemElement : match
        .getMapContainer()
        .getMapInfo()
        .getJsonObject()
        .getAsJsonArray("itemremove")) {
        try {
          // 1.13 temp fix
          if (
            Strings
              .getTechnicalName(itemElement.getAsString())
          ...
          add(
            Material.valueOf(
              Strings.getTechnicalName(itemElement.getAsString())
            )
          );
        } catch (Exception e) {
          TGM
            ...
            .broadcastToAdmins(
              ChatColor.RED +
              "[JSON] Unknown material in itemremove module: \"" +
  ...
  public void onDrop(PlayerDropItemEvent event) {
    ...
    ItemRemoveInfo info = hasMaterial(
      event.getItemDrop().getItemStack().getType()
    );
    if (info != null && info.isPreventingPlayerDrop()) event
      .getItemDrop()
      .remove();
  }
  ...
  public static class ItemRemoveInfo {
    ...
    public ItemRemoveInfo setPreventingPlayerDrop(
      boolean preventingPlayerDrop
    ) {
