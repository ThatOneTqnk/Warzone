...
public class HemisphereRegion implements Region {
  ...
  public HemisphereRegion(
    Location focalPoint,
    double radius,
    HemisphereFace hemisphereFace
  ) {
      ...
      case POSITIVE_X:
        this.min =
          new Location(
            focalPoint.getWorld(),
            focalPoint.getX(),
            focalPoint.getY() - radius,
            focalPoint.getZ() - radius
          );
        this.max =
          new Location(
            focalPoint.getWorld(),
            focalPoint.getX() + radius,
            focalPoint.getY() + radius,
            focalPoint.getZ() + radius
          );
        break;
  ...
  public boolean contains(Location location) {
    return (
      hemisphereFace.hemisphereDirectionEvaluation.contains(
        focalPoint,
        location
      ) &&
      focalPoint.distance(location) <= radius
    );
  }
  ...
  public static HemisphereFace parseHemisphereDirection(
    JsonElement hemisphereDirection
  ) {
    String hemisphereDirRaw = hemisphereDirection.getAsString();
    return HemisphereFace.valueOf(
      (hemisphereDirRaw.charAt(0) == '-' ? "NEGATIVE" : "POSITIVE") +
      "_" +
      Character.toString(hemisphereDirRaw.charAt(1)).toUpperCase()
    );
  }
