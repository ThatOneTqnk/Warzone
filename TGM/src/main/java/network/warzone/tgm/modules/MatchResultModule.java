...
public class MatchResultModule extends MatchModule implements Listener {
    ...
    for (Player player : Bukkit.getOnlinePlayers()) {
      ...
      if (killstreak >= 5) {
        player.sendMessage(
          ChatColor.translateAlternateColorCodes(
            '&',
            "&cYour killstreak of &4" + killstreak + "&c ended."
          )
        );
      }
      ...
      if (spectators.containsPlayer(player)) {
        ...
        if (event.getWinningTeam() == null) {
          player.sendTitle(
            "",
            ChatColor.YELLOW + "The result was a tie!",
            10,
            40,
            10
          );
      ...
      if (event.getWinningTeam() != null) {
        player.sendMessage(
          ...
          "  Winner: " +
          event.getWinningTeam().getColor() +
          event.getWinningTeam().getAlias()
        );
      } else {
        player.sendMessage(
          ChatColor.DARK_PURPLE +
      ...
      if (
        ...
        event.getWinningTeam().containsPlayer(player)
      ) {
        player.sendMessage(ChatColor.GRAY + "  Congratulations!");
      } else if (
        TGM.get().getModule(TeamManagerModule.class).getTeam(player) != null &&
        TGM
          .get()
          .getModule(TeamManagerModule.class)
          .getTeam(player)
  ...
  private void applyPostPlayer(Player player) {
    if (
      TGM.get().getConfig().getBoolean("map.post-block-break", false) &&
      player.hasPermission("tgm.post.break")
    ) {
    ...
    player.addPotionEffect(
      new PotionEffect(PotionEffectType.WEAKNESS, 1000000, 5, true, false),
      true
    );
  }
  ...
  public void onMatchResult(PlayerJoinTeamAttemptEvent event) {
    if (match.get().getMatchStatus().equals(MatchStatus.POST)) {
      event
        .getPlayerContext()
        .getPlayer()
        .sendMessage(ChatColor.RED + "The match has already ended.");
      event.setCancelled(true);
    }
  }
}
