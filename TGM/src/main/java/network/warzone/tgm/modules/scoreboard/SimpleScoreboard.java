...
public class SimpleScoreboard {
  ...
  private final UUID invalidUserUUID = UUID.nameUUIDFromBytes(
    "InvalidUsername".getBytes(Charsets.UTF_8)
  );
  ...
  private Map.Entry<Team, OfflinePlayer> createTeam(String text, int pos) {
    ...
    if (!cache.containsKey(color.toString())) cache.put(
      color.toString(),
      getOfflinePlayerSkipLookup(color.toString())
    );
  ...
  private void applyText(Team team, String text, OfflinePlayer result) {
    ...
    if (text.length() > 16) {
      ...
      team.setSuffix(
        ("".equals(prefixColor) ? ChatColor.RESET : prefixColor) + suffix
      );
    }
  }
  ...
  public void update() {
    ...
    for (Map.Entry<String, Integer> text : scores.entrySet()) {
      Team t = scoreboard.getTeam(
        ChatColor.values()[text.getValue()].toString()
      );
      ...
      if (t != null) {
        ...
        if (!cache.containsKey(color)) cache.put(
          color,
          getOfflinePlayerSkipLookup(color)
        );
  ...
  private OfflinePlayer getOfflinePlayerSkipLookup(String name) {
    try {
      if (craftOfflinePlayerConstructor == null) {
        Class<?> craftOfflinePlayerClass = Class.forName(
          Bukkit
            .getServer()
            .getClass()
            .getName()
        ...
        craftOfflinePlayerConstructor =
          craftOfflinePlayerClass.getDeclaredConstructor(
      ...
      Object craftOfflinePlayer = craftOfflinePlayerConstructor.newInstance(
        Bukkit.getServer(),
        gameProfile
      );
