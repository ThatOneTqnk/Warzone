...
public class InventoryPreviewModule
  extends MatchModule
  implements Listener, TaskedModule {
  ...
  public void load(Match match) {
    ...
    if (jsonObject.has("inventoryPreview")) {
      this.enabled =
        !jsonObject.getAsJsonObject("inventoryPreview").has("enabled") ||
        jsonObject
          .getAsJsonObject("inventoryPreview")
          .get("enabled")
          .getAsBoolean();
    }
  ...
  @Override
  public void tick() {
    inventoryClones.keySet().forEach(this::updateInventory);
  }
  ...
  public void onClose(InventoryCloseEvent event) {
    ...
    if (
      !this.playersViewingInventoryClones.containsValue(event.getInventory())
    ) {
  ...
  private void updateInventory(Inventory real) {
    ...
    if (real instanceof PlayerInventory) {
      ...
      if (player != null) InventoryUtil.setPlayerInventoryContents(
        player,
        clone
      );
    } else {
  ...
  public boolean isSpectating(Player player) {
    ...
    return (
      ...
      spectatorModule.getSpectators().containsPlayer(player)
    );
  }
}
