...
public class FilterManagerModule extends MatchModule {
  ...
  public void enable() {
    if (
      match.get().getMapContainer().getMapInfo().getJsonObject().has("filters")
    ) {
      for (JsonElement filterElement : match
        .get()
        .getMapContainer()
        .getMapInfo()
        .getJsonObject()
  ...
  private List<FilterType> initFilter(Match match, JsonObject jsonObject) {
    ...
    if ("build".equals(type)) filterTypes.add(
      BuildFilterType.parse(match, jsonObject)
    ); else if ("enter".equals(type)) filterTypes.add(
      EnterFilterType.parse(match, jsonObject)
    ); else if ("usebow".equals(type)) filterTypes.add(
      UseBowFilterType.parse(match, jsonObject)
    ); else if ("useshear".equals(type)) filterTypes.add(
      UseShearFilterType.parse(match, jsonObject)
    ); else if ("leave".equals(type)) filterTypes.add(
      LeaveFilterType.parse(match, jsonObject)
    ); else if ("blockexplode".equals(type)) filterTypes.add(
      BlockExplodeFilterType.parse(match, jsonObject)
    ); else if ("blockplace".equals(type)) filterTypes.add(
      BlockPlaceFilterType.parse(match, jsonObject)
    ); else if ("blockbreak".equals(type)) filterTypes.add(
      BlockBreakFilterType.parse(match, jsonObject)
    ); else if ("voidbuild".equals(type)) filterTypes.add(
      VoidBuildFilterType.parse(match, jsonObject)
    );
