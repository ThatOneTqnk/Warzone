...
public class ChatModule extends MatchModule implements Listener {
  ...
  private static final List<String> blockedCmds = Arrays.asList(
    "t ",
    "w ",
    "r ",
    "reply",
  ...
  public void onCommand(PlayerCommandPreprocessEvent event) {
    ...
    if (
      playerContext.getUserProfile().getLatestMute() != null &&
      playerContext.getUserProfile().getLatestMute().isActive() &&
  ...
  private boolean startsWith(String msg) {
    for (String cmd : blockedCmds) {
      if (
        msg.toLowerCase().startsWith("/" + cmd) ||
        msg.toLowerCase().startsWith(cmd)
      ) return true;
    }
    return false;
  ...
  public void onChat(AsyncPlayerChatEvent event) {
    ...
    if (
      !TGM.get().getConfig().getBoolean("chat.enabled") &&
      !event.getPlayer().hasPermission("tgm.chat.bypass")
    ) {
      event
        .getPlayer()
        .sendMessage(
          ChatColor.translateAlternateColorCodes(
            '&',
            TGM.get().getConfig().getString("chat.messages.muted")
          )
        );
      event.setCancelled(true);
      return;
    ...
    if (channel == ChatChannel.TEAM) {
      TGM
        ...
        .sendTeamChat(playerContext, event.getMessage());
      event.setCancelled(true);
      return;
    }
    ...
    if (channel == ChatChannel.ALL) {
      ...
      String prefix = userProfile.getPrefix() != null
        ? ChatColor.translateAlternateColorCodes(
          '&',
          userProfile.getPrefix().trim()
        ) +
        " "
        : "";
      StringBuilder format = new StringBuilder();
      if (!TGM.get().getModule(StatsModule.class).isStatsDisabled()) format
        .append(playerContext.getLevelString())
        .append(" ");
      format
        .append(prefix)
      ...
      if (
        !playerContext.isNicked() &&
        userProfile.getActiveTag() != null &&
        !"".equals(userProfile.getActiveTag())
      ) format
  ...
  public static void sendStaffMessage(
    String prefix,
    String sender,
    String message
  ) {
  ...
  public void onChatHighPriority(AsyncPlayerChatEvent event) {
    ...
    if (!event.isCancelled()) {
          ...
          player -> {
            BaseComponent[] stats = new BaseComponent[] {
              new TextComponent(
                ChatColor.AQUA +
                "Level: " +
                playerContext.getLevelString().replace("[", "").replace("]", "")
              ),
              new TextComponent("\n"),
              new TextComponent(
                "\n" +
                ...
                "XP: " +
                ChatColor.RESET +
                playerContext.getUserProfile().getXP()
              ),
              new TextComponent(
                "\n" +
                ChatColor.AQUA +
                "Kills: " +
                ChatColor.RESET +
                playerContext.getUserProfile().getKills()
              ),
              new TextComponent(
                "\n" +
                ChatColor.AQUA +
                "Deaths: " +
                ChatColor.RESET +
                playerContext.getUserProfile().getDeaths()
              ),
              new TextComponent(
                "\n" +
                ChatColor.AQUA +
                "K/D: " +
                ChatColor.RESET +
                playerContext.getUserProfile().getKDR()
              ),
              new TextComponent("\n"),
              new TextComponent(
                "\n" +
                ...
                "Wins: " +
                ChatColor.RESET +
                playerContext.getUserProfile().getWins()
              ),
              new TextComponent(
                "\n" +
                ChatColor.AQUA +
                "Losses: " +
                ChatColor.RESET +
                playerContext.getUserProfile().getLosses()
              ),
              new TextComponent(
                "\n" +
                ChatColor.AQUA +
                "W/L: " +
                ChatColor.RESET +
                playerContext.getUserProfile().getWLR()
              ),
            };
            ...
            BaseComponent[] mainComponents = TextComponent.fromLegacyText(
              String.format(
                event.getFormat(),
                playerContext.getPlayer().getName(),
                event.getMessage()
              )
            );
      ...
      Bukkit
        .getConsoleSender()
        .sendMessage(event.getFormat().replace("%%", "%"));
    }
    event.setCancelled(true);
  }
