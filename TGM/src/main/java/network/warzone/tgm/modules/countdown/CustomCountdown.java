...
public class CustomCountdown extends BossBarCountdown {
  ...
  public void onTeamChange(TeamChangeEvent event) {
    if (
      this.teams == null || this.teams.isEmpty() || event.isCancelled()
    ) return;
    if (this.teams.contains(event.getTeam())) {
      if (!this.teams.contains(event.getOldTeam())) bossBar.addPlayer(
        event.getPlayerContext().getPlayer()
      );
    } else {
      if (this.teams.contains(event.getOldTeam())) bossBar.removePlayer(
        event.getPlayerContext().getPlayer()
      );
    }
  }
  ...
  private double getProgress() {
    double progress;
    if (!this.invert) progress =
      (getTimeMax() - getTimeLeft()) / getTimeMax(); else progress =
      getTimeLeft() / getTimeMax();
    if (progress > 1) return 1.0;
    if (progress < 0) return 0.0; else return progress;
  }
  ...
  private String getFormattedTitle() {
    ...
    return ChatColor.translateAlternateColorCodes(
      '&',
      Placeholders.apply(this.title, placeholders)
    );
  }

  private List<Player> getPlayers() {
    if (this.teams == null || this.teams.isEmpty()) return new ArrayList<>(
      Bukkit.getOnlinePlayers()
    ); else return this.teams.stream()
  ...
  private void addPlayers() {
    getPlayers()
      .stream()
      .filter(player -> !getBossBar().getPlayers().contains(player))
      .forEach(player -> getBossBar().addPlayer(player));
  }
  ...
  public void onUpdate() {
    ...
    getBossBar()
      .getPlayers()
      .forEach(player -> getBossBar().removePlayer(player));
    addPlayers();
  }
