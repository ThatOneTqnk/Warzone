 ...
 */
public class CTFTimeController
  extends CTFController
  implements TimeLimitService, TimeSubscriber {
  ...
  public CTFTimeController(
    CTFControllerSubscriber subscriber,
    List<MatchFlag> allFlags,
    int timeLimit
  ) {
  ...
  private void updateScoreboard(
    ...
    scoreboard.add(
      "Time: " + ChatColor.GREEN + formattedRemainingTime,
    ...
    for (MatchTeam team : teamManagerModule.getTeams()) {
      ...
      scoreboard.add(
        ChatColor.LIGHT_PURPLE.toString() + getTeamPoints(team) + " points",
        ++positionOnScoreboard
      );
    ...
    for (MatchFlag flag : allFlags) {
      ...
      scoreboard.add(
        ...
        CTFModule.RIGHT_ARROW +
        " " +
        team.getColor() +
        flag.getFlagHolder().getName(),
        ++positionOnScoreboard
      );
    }
    if (addedAnyFlags) scoreboard.add(
      StringUtils.repeat(" ", ++spaceCount),
      ++positionOnScoreboard
    );
    scoreboard.update();
  ...
  private void updateAllScoreboards(String remainingTime) {
    for (SimpleScoreboard simpleScoreboard : scoreboardManagerModule
      .getScoreboards()
      .values()) {
