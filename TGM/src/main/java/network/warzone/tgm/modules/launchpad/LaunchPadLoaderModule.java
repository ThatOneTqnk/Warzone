 ...
 */
@ModuleData(load = ModuleLoadTime.LATEST)
@Getter
public class LaunchPadLoaderModule extends MatchModule {
  ...
  public void load(Match match) {
    JsonObject jsonObject = match
      ...
      .getJsonObject();
    if (
      jsonObject.has("launchpads") && jsonObject.get("launchpads").isJsonArray()
    ) {
      for (JsonElement element : jsonObject.getAsJsonArray("launchpads")) {
        if (!element.isJsonObject()) continue;
        try {
          LaunchPadModule launchPad = LaunchPadModule.deserialize(
