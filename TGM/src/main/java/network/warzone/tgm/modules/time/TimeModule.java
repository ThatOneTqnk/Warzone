...
public class TimeModule extends MatchModule {
  ...
  @Setter
  private int timeLimit = 20 * 60; // Default (20 minutes)
  ...
  public void load(Match match) {
    if (match.getMapContainer().getMapInfo().getJsonObject().has("time")) {
      ...
      if (timeObject.has("defaultWinner")) defaultWinner =
        match
          .getModule(TeamManagerModule.class)
          .getTeamById(timeObject.get("defaultWinner").getAsString());
      if (
        timeObject.has("broadcasts") &&
        timeObject.get("broadcasts").isJsonArray()
      ) {
        for (JsonElement element : timeObject.getAsJsonArray("broadcasts")) {
          ...
          String message = broadcast.has("message")
            ? broadcast.get("message").getAsString()
            : null;
          List<String> commands = new ArrayList<>();
          for (JsonElement cmdElement : broadcast
            .get("commands")
            .getAsJsonArray()) {
          ...
          if (
            broadcast.has("exclude") && broadcast.get("exclude").isJsonArray()
          ) broadcast
            .get("exclude")
            .getAsJsonArray()
            .forEach(jsonElement -> exclude.add(jsonElement.getAsInt()));
          broadcasts.add(
            new Broadcast(message, commands, interval, repeat, exclude)
          );
        }
      }
    }
