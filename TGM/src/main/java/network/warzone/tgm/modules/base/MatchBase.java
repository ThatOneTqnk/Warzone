...
public class MatchBase implements Listener {
  ...
  public MatchBase(
    Region baseRegion,
    MatchTeam team,
    List<? extends Redeemable> redeemables
  ) {
  ...
  public void onBlockBreak(BlockBreakEvent event) {
    ...
    event
      .getPlayer()
      .sendMessage(ChatColor.RED + "You cannot break near a capture base!");
  ...
  public void onBlockPlace(BlockPlaceEvent event) {
    ...
    event
      .getPlayer()
      .sendMessage(ChatColor.RED + "You cannot build near a capture base!");
    event.setCancelled(true);
  }
  ...
  private List<PlayerRedeemable> hasPlayerRedeemables(Player player) {
    ...
    for (PlayerRedeemable playerRedeemable : playerRedeemables) {
      if (playerRedeemable.hasRedeemable(player)) eligibleRedeemables.add(
        playerRedeemable
      );
    }
  ...
  private List<ItemRedeemable> applicableItemRedeemables(ItemStack item) {
    ...
    for (ItemRedeemable itemRedeemable : itemRedeemables) {
      if (itemRedeemable.matchesRedeemable(item)) eligibleRedeemables.add(
        itemRedeemable
      );
    }
    return eligibleRedeemables;
  ...
  public void onMove(PlayerMoveEvent event) {
    if (
      match.getMatchStatus() != MatchStatus.MID ||
      ...
      !baseRegion.contains(event.getFrom())
    ) return; else {
      List<PlayerRedeemable> eligiblePlayerRedeemables = hasPlayerRedeemables(
        event.getPlayer()
      );
      List<ItemRedeemable> eligibleItemRedeemables = applicableItemRedeemables(
        event.getPlayer().getActiveItem()
      );
      if (
        eligiblePlayerRedeemables.size() == 0 &&
        eligibleItemRedeemables.size() == 0
      ) return;
  ...
  private void redeemPlayerRedeemables(
    List<PlayerRedeemable> filteredPlayerRedeemables,
    Player player
  ) {
    for (PlayerRedeemable playerRedeemable : filteredPlayerRedeemables) playerRedeemable.redeem(
      player
    );
  }

  private void redeemItemRedeemables(
    List<ItemRedeemable> filteredItemRedeemables,
    Player player
  ) {
    for (ItemRedeemable itemRedeemable : filteredItemRedeemables) itemRedeemable.redeem(
      player
    );
  }
