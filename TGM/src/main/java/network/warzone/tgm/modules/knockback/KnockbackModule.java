...
public class KnockbackModule extends MatchModule implements Listener {
  ...
  private static void loadValues() {
    enabled =
      TGM.get().getConfig().getBoolean("custom-knockback.enabled", false);
    knockBackFriction =
      TGM.get().getConfig().getDouble("custom-knockback.friction");
    knockBackHorizontal =
      TGM.get().getConfig().getDouble("custom-knockback.horizontal");
    knockBackVertical =
      TGM.get().getConfig().getDouble("custom-knockback.vertical");
    knockBackVerticalLimit =
      TGM.get().getConfig().getDouble("custom-knockback.vertical-limit");
    knockBackExtraHorizontal =
      TGM.get().getConfig().getDouble("custom-knockback.horizontal-extra");
    knockBackExtraVertical =
      TGM.get().getConfig().getDouble("custom-knockback.vertical-extra");
    knockBackBowBase =
      TGM.get().getConfig().getDouble("custom-knockback.bow-base");
    knockBackBowVertical =
      TGM.get().getConfig().getDouble("custom-knockback.bow-vertical");
    knockBackPunchMultiplier =
      TGM.get().getConfig().getDouble("custom-knockback.punch-multiplier");
  }
  ...
  public void onEntityDamage(EntityDamageByEntityEvent event) {
    ...
    this.queued.put(
        player,
        new EntityDamageByEntityContext(
          event,
          event.getDamager() instanceof Player &&
          ((Player) event.getDamager()).isSprinting()
        )
      );
  }
  ...
  public void onPlayerVelocity(PlayerVelocityEvent event) {
    ...
    if (CAUSES.contains(edEvent.getCause())) {
      ...
      if (
        edEvent.getCause() == EntityDamageEvent.DamageCause.ENTITY_ATTACK &&
        edEvent.getDamager() instanceof LivingEntity
      ) {
        event.setVelocity(new Vector());
        applyMeleeKnockback(event, edEvent.getDamager(), context.isSprinting());
      } else if (
        damager instanceof Arrow && this.arrowDirection.containsKey(damager)
      ) {
        event.setVelocity(new Vector());
    ...
    try {
      ...
      if (this.random.nextDouble() >= kbRes) {
        ...
        if (distance > 0) {
          velocity.setX(
            velocity.getX() +
            (d0 * knockBackBowBase / distance) +
            (d0 * punch * knockBackPunchMultiplier) /
            distance
          );
          velocity.setY(knockBackBowVertical);
          velocity.setZ(
            velocity.getZ() +
            (d1 * knockBackBowBase / distance) +
            (d1 * punch * knockBackPunchMultiplier) /
            distance
          );
        }
  ...
  private void applyMeleeKnockback(
    ...
    try {
      ...
      if (this.random.nextDouble() >= kbRes) {
        Vector velocity = getBaseKnockback(
          event.getPlayer(),
          event.getVelocity(),
          attacker
        );
        ...
        Vector kb = new Vector(
          ...
          event.getPlayer().isOnGround()
            ? knockBackExtraVertical + vertical
            : 0,
  ...
  private Vector getBaseKnockback(
    LivingEntity victim,
    Vector vector,
    Entity attacker
  ) {
    ...
    for (
      d1 = attacker.getLocation().getZ() - victim.getLocation().getZ();
      d0 * d0 + d1 * d1 < 1.0E-4D;
