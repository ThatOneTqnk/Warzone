...
public class GeneratorHologram {
  ...
  public GeneratorHologram(
    ...
    this.holoAnchor =
      location
        .getWorld()
        .spawn(location.clone().add(0, 1.6, 0), ArmorStand.class);
  ...
  public static GeneratorHologram deserialize(
    JsonObject generatorHologramObject,
    Location holoLocation
  ) {
    assert generatorHologramObject.has(
      "content"
    ) : "GeneratorHologram needs content";
    String hologramContent = ChatColor.translateAlternateColorCodes(
      '&',
      generatorHologramObject.get("content").getAsString()
    );
    TimeDisplay hologramTimeDisplay = TimeDisplay.SECONDS;
    if (generatorHologramObject.has("timeUnit")) hologramTimeDisplay =
      TimeDisplay.valueOf(
        Strings.getTechnicalName(
          generatorHologramObject.get("timeUnit").getAsString()
        )
      );
    return new GeneratorHologram(
      hologramContent,
      hologramTimeDisplay,
      holoLocation
    );
