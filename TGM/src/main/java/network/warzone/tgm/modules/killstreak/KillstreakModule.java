...
public class KillstreakModule extends MatchModule implements Listener {
  ...
  public void load(Match match) {
    ...
    this.addDefaults();
    if (
      match.getMapContainer().getMapInfo().getJsonObject().has("killstreaks")
    ) {
      Killstreak[] parsedKillstreaks = TGM
        .get()
        .getGson()
        .fromJson(
          match
  ...
  public void onKill(TGMPlayerDeathEvent event) {
    ...
    if (deathInfo.killer == null) {
      if (
        players.getOrDefault(deathInfo.player.getUniqueId().toString(), 0) >= 5
      ) {
        Bukkit.broadcastMessage(
          ColorConverter.filterString(
            deathInfo.playerTeam.getColor().toString() +
            deathInfo.playerName +
            "&7" +
            (deathInfo.playerName.endsWith("s") ? "'" : "'s") +
            " kill streak of &c&l" +
            players.get(deathInfo.player.getUniqueId().toString()) +
            "&r&7 was shutdown"
          )
        );
      }
    ...
    if (players.get(killedUuid) != null && players.get(killedUuid) >= 5) {
      Bukkit.broadcastMessage(
        ColorConverter.filterString(
          deathInfo.killerTeam.getColor().toString() +
    ...
    killstreaks.forEach(
      killstreak -> {
        if (
          !killstreak.isRepeat() &&
          players.get(killerUuid) == killstreak.getCount() ||
          killstreak.isRepeat() &&
          players.get(killerUuid) % killstreak.getCount() == 0
        ) {
          if (
            killstreak.getMessage() != null &&
            !killstreak.getMessage().isEmpty()
          ) Bukkit.broadcastMessage(
            ColorConverter
              .filterString(killstreak.getMessage())
  ...
  public void onTeamChange(TeamChangeEvent event) {
    ...
    if (
      players.containsKey(
        event.getPlayerContext().getPlayer().getUniqueId().toString()
      ) &&
      players.get(
        event.getPlayerContext().getPlayer().getUniqueId().toString()
      ) >=
      ...
      event
        ...
        .sendMessage(
          ColorConverter.filterString(
            "&7You have lost your kill streak of &c&l"
          ) +
          streakValue +
          ColorConverter.filterString(" &7for switching teams.")
        );
      players.remove(
        event.getPlayerContext().getPlayer().getUniqueId().toString()
      );
  ...
  private void addDefaults() {
    killstreaks.addAll(
      Arrays.asList( //TODO Add option to disable it
        new Killstreak()
          .setCount(5)
          .setMessage(
            "%killercolor%%killername% &7is on a kill streak of &2&l%count%&r&7!"
          )
          .setActions(
            Collections.singletonList(
              new SoundKillstreakAction(
                Sound.ENTITY_ZOMBIE_DEATH,
                SoundKillstreakAction.SoundTarget.EVERYONE,
                3.0F,
                1.0F
              )
            )
        ...
        new Killstreak()
          .setCount(10)
          .setMessage(
            "%killercolor%%killername% &7is on a kill streak of &6&l%count%&r&7!"
          )
          .setActions(
            Arrays.asList(
              new SoundKillstreakAction(
              ...
              new FireworkKillstreakAction(
                new Location(match.get().getWorld(), 0.0, 0.0, 0.0),
                FireworkEffect
                  .builder()
                  .with(FireworkEffect.Type.CREEPER)
                  .withColor(Color.RED)
                  .withFade(Color.fromRGB(9371648))
                  .build(),
                0
              )
            )
        ...
        new Killstreak()
          .setCount(25)
          .setMessage(
            "%killercolor%%killername% &7is on a kill streak of &c&l%count%&r&7!"
          )
          .setActions(
            Arrays.asList(
              new SoundKillstreakAction(
        ...
        new Killstreak()
          .setCount(50)
          .setMessage(
            "%killercolor%%killername% &7is on a kill streak of &3&l%count%&r&7!"
          )
          .setActions(
            Arrays.asList(
              new SoundKillstreakAction(
        ...
        new Killstreak()
          .setCount(100)
          .setMessage(
            "%killercolor%%killername% &7is on a kill streak of &5&l%count%&r&7!"
          )
          .setActions(
            Arrays.asList(
              new SoundKillstreakAction(
                Sound.UI_TOAST_CHALLENGE_COMPLETE,
                SoundKillstreakAction.SoundTarget.PLAYER,
                1000.0F,
                1.0F
              ),
              new FireworkKillstreakAction(
                ...
                FireworkEffect
                  ...
                  .withColor(Color.fromRGB(16766776))
                  .withFade(Color.fromRGB(16774912))
                  .build(),
                0
              )
            )
