...
public class BlitzModule extends MatchModule implements Listener {
  ...
  private String subtitle =
    ChatColor.GREEN + "Remaining lives:  " + ChatColor.YELLOW + "%lives%";
  private String actionbar = "";

  private WeakReference<Match> match;

  private final RespawnRule respawnRule = new RespawnRule(
    null,
    3000,
    false,
    false,
  ...
  public void load(Match match) {
    ...
    if (mapInfo.has("blitz")) {
      ...
      if (blitz.has("deathTitle")) title =
        blitz.get("deathTitle").getAsString();
      if (blitz.has("deathSubtitle")) subtitle =
        blitz.get("deathSubtitle").getAsString();
      if (blitz.has("actionbar")) actionbar =
        blitz.get("actionbar").getAsString();
      for (JsonElement teamElement : blitz.getAsJsonArray("lives")) {
        JsonObject teamObject = (JsonObject) teamElement;
        MatchTeam team = teamManagerModule.getTeamById(
          teamObject.get("team").getAsString()
        );
    ...
    TGM
      ...
      .setTimeLimitService(this::getBiggestTeam);
  ...
  private MatchTeam getBiggestTeam() {
    ...
    if (highest != null) {
      ...
      int amount = (int) teamManagerModule
        .getTeams()
        .stream()
        .filter(
          t -> getAlivePlayers(team) == getAlivePlayers(t) && !t.isSpectator()
        )
        .count();
      if (amount > 1) return null; else return team;
    }
    return null;
  }
            ...
            for (Player player : Bukkit.getOnlinePlayers()) {
              ...
              player.sendActionBar(
                ChatColor.translateAlternateColorCodes(
                  '&',
                  actionbar
                    .replaceAll("%lives%", "" + getLives(player))
  ...
  public void onScoreboardInit(ScoreboardInitEvent event) {
    ...
    for (MatchTeam matchTeam : teams) {
      ...
      simpleScoreboard.add(
        matchTeam.getColor() +
        getTeamScoreLine(matchTeam, getAlivePlayers(matchTeam).size()),
        i
      );
  ...
  public void onJoinAttempt(PlayerJoinTeamAttemptEvent event) {
    if (!this.match.get().getMatchStatus().equals(MatchStatus.PRE)) {
      event
        ...
        .sendMessage(
          ChatColor.RED +
          "You can't pick a team after the match starts in this mode."
        );
  ...
  public void updateScoreboardTeamLine(MatchTeam matchTeam, int size) {
    ...
    for (SimpleScoreboard simpleScoreboard : TGM
      .get()
  ...
  public void onDeath(TGMPlayerDeathEvent event) {
    ...
    if (getLives(player) <= 0) {
      ...
      showLives(player);
      player.teleport(
        teamManagerModule.getTeam(player).getSpawnPoints().get(0).getLocation()
      );
    }
  }
  ...
  public void onDeathHigh(TGMPlayerDeathEvent event) {
    if (lastTeamAlive()) {
      MatchTeam winnerTeam = teamManagerModule
        .getTeams()
        .stream()
        .filter(matchTeam -> !matchTeam.isSpectator())
        .filter(matchTeam -> getAlivePlayers(matchTeam).size() > 0)
        .findFirst()
  ...
  private void handleQuit(PlayerEvent event) {
    if (
      (
        teamManagerModule.getTeam(event.getPlayer()) != null &&
        teamManagerModule.getTeam(event.getPlayer()).isSpectator()
      )
    ) return;
    updateScoreboardTeamLine(
      teamManagerModule.getTeam(event.getPlayer()),
      getAlivePlayers(teamManagerModule.getTeam(event.getPlayer())).size() - 1
    );
  ...
  public void onTeamChange(TeamChangeEvent event) {
    ...
    if (
      event.getOldTeam() != null && !event.getOldTeam().isSpectator()
    ) updateScoreboardTeamLine(
      event.getOldTeam(),
      getAlivePlayers(event.getOldTeam()).size()
    );
    if (!event.getTeam().isSpectator()) updateScoreboardTeamLine(
      event.getTeam(),
      getAlivePlayers(event.getTeam()).size()
    );

    playerLives.put(
      event.getPlayerContext().getPlayer().getUniqueId(),
      teamLives.getOrDefault(event.getTeam(), 1)
    );
  }
  ...
  private void removeLife(Player player) {
    playerLives.put(
      player.getUniqueId(),
      (
        playerLives.getOrDefault(player.getUniqueId(), 0) > 0
          ? playerLives.get(player.getUniqueId()) - 1
          : 0
      )
    );
  }
  ...
  private List<PlayerContext> getAlivePlayers(MatchTeam matchTeam) {
    return matchTeam
      .getMembers()
      .stream()
      .filter(playerContext -> isAlive(playerContext.getPlayer()))
