...
public class ScheduledGeneratorUpgrader extends GeneratorUpgrader {
  ...
  public ScheduledGeneratorUpgrader(
    List<ScheduledGeneratorUpgrade> scheduledGeneratorUpgrades
  ) {
  ...
  public void enable() {
    for (ScheduledGeneratorUpgrade scheduledGeneratorUpgrade : scheduledGeneratorUpgrades) {
      ...
      taskMap.put(
        scheduledGeneratorUpgrade.getTime(),
        Bukkit
          .getScheduler()
          .runTaskLater(
            TGM.get(),
  ...
  private void applyUpgrade(
    ScheduledGeneratorUpgrade scheduledGeneratorUpgrade
  ) {
    if (scheduledGeneratorUpgrade.getItem() != null) hostGenerator.setItem(
      scheduledGeneratorUpgrade.getItem()
    );
    if (scheduledGeneratorUpgrade.getInterval() > 0) hostGenerator.setInterval(
      scheduledGeneratorUpgrade.getInterval()
    );
    if (
      scheduledGeneratorUpgrade.getBroadcast() != null
    ) Bukkit.broadcastMessage(
      parseCurrentBroadcast(scheduledGeneratorUpgrade.getBroadcast())
    );
    if (
      scheduledGeneratorUpgrade.getHoloContent() != null &&
      hostGenerator.getGeneratorHologram() != null
    ) hostGenerator
      .getGeneratorHologram()
      .setBaseContent(scheduledGeneratorUpgrade.getHoloContent());
  }
  ...
  public static ScheduledGeneratorUpgrader deserialize(
    JsonObject upgradeObject
  ) {
    List<ScheduledGeneratorUpgrade> scheduledGeneratorUpgrades = new ArrayList<>();
    for (JsonElement upgradeSequenceElement : upgradeObject
      .get("sequence")
      .getAsJsonArray()) {
      ...
      int interval = -1;
      if (sequenceObject.has("interval")) interval =
        sequenceObject.get("interval").getAsInt();
      ItemStack item = null;
      if (sequenceObject.has("item")) item =
        ItemDeserializer.parse(sequenceObject.get("item").getAsJsonObject());
      String message = null;
      if (sequenceObject.has("message")) message =
        ChatColor.translateAlternateColorCodes(
          '&',
          sequenceObject.get("message").getAsString()
        );
      String holoContent = null;
      if (sequenceObject.has("holoContent")) holoContent =
        ChatColor.translateAlternateColorCodes(
          '&',
          sequenceObject.get("holoContent").getAsString()
        );
      scheduledGeneratorUpgrades.add(
        new ScheduledGeneratorUpgrade(
          time,
          interval,
          item,
          message,
