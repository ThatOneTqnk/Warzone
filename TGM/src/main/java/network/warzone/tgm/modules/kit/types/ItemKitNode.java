...
public class ItemKitNode implements KitNode {
  ...
  public void apply(Player player, MatchTeam matchTeam) {
    ...
    if (
      slot >= 100 &&
      !hasColor &&
      itemStack.getType().toString().contains("LEATHER_")
    ) {
    ...
    if (slot == 100) player.getInventory().setBoots(itemStack); else if (
      slot == 101
    ) player.getInventory().setLeggings(itemStack); else if (slot == 102) player
      .getInventory()
      .setChestplate(itemStack); else if (slot == 103) player
      .getInventory()
      .setHelmet(itemStack); else if (slot == -106) player
      .getInventory()
      .setItemInOffHand(itemStack); else player
      .getInventory()
