...
public class TeamManagerModule extends MatchModule implements Listener {
  ...
  public void load(Match match) {
    ...
    for (ParsedTeam parsedTeam : match
      .getMapContainer()
      .getMapInfo()
      .getTeams()) {
      teams.add(
  ...
  public void onMatchJoin(MatchJoinEvent event) {
    MatchTeam matchTeam = teamJoinController.determineTeam(
  ...
  public void joinTeam(
    ...
    TeamChangeEvent event = new TeamChangeEvent(
      playerContext,
      matchTeam,
      oldTeam,
      false,
  ...
  public MatchTeam getSmallestTeam() {
    ...
    for (MatchTeam matchTeam : teams) {
      if (!matchTeam.isSpectator()) {
        ...
        double teamSize =
          ((double) matchTeam.getMembers().size()) / matchTeam.getMax();
        double smallestTeamSize =
          ((double) smallest.getMembers().size()) / smallest.getMax();
        if (teamSize < smallestTeamSize) {
          smallest = matchTeam;
          continue;
        }
        if (
          teamSize == smallestTeamSize && new Random().nextBoolean()
        ) smallest = matchTeam;
      }
    }
