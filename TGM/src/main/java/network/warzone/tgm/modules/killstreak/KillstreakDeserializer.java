...
public class KillstreakDeserializer implements JsonDeserializer<Killstreak> {
  ...
  public Killstreak deserialize(
    ...
    if (streakJson.has("commands")) {
      killstreak.setCommands(
        new ArrayList<String>() {
          {
            streakJson
              .getAsJsonArray("commands")
              .forEach(jsonElement -> add(jsonElement.getAsString()));
          }
        }
      );
    }
    ...
    if (streakJson.has("actions")) {
      ...
      if (actionObj.has("fireworks")) {
        for (JsonElement jsonElem : actionObj.getAsJsonArray("fireworks")) {
          ...
          String fireworkType = fireworkObj.has("type")
            ? Strings.getTechnicalName(fireworkObj.get("type").getAsString())
            : "BALL";
          boolean shouldTrail =
            fireworkObj.has("trail") && fireworkObj.get("trail").getAsBoolean();
          boolean shouldFlicker =
            fireworkObj.has("flicker") &&
            fireworkObj.get("flicker").getAsBoolean();
          ...
          int fireworkLifetime = fireworkObj.has("lifetime")
            ? fireworkObj.get("lifetime").getAsInt()
            : 0;
          Location locationOffset = fireworkObj.has("locationOffset")
            ? Parser.convertLocation(
              match.getWorld(),
              fireworkObj.get("locationOffset")
            )
            : new Location(match.getWorld(), 0.0, 0.0, 0.0);
          killstreakActions.add(
      ...
      if (actionObj.has("sounds")) {
        for (JsonElement jsonElem : actionObj.getAsJsonArray("sounds")) {
          ...
          Sound theSound = Sound.valueOf(
            soundObj.get("sound").getAsString().toUpperCase().replace(".", "_")
          );
          float volume = soundObj.has("volume")
            ? soundObj.get("volume").getAsFloat()
            : 1.0F;
          float pitch = soundObj.has("pitch")
            ? soundObj.get("pitch").getAsFloat()
            : 1.0F;
          SoundKillstreakAction.SoundTarget soundTarget = soundObj.has("target")
            ? SoundKillstreakAction.SoundTarget.valueOf(
              soundObj
                .get("target")
                .getAsString()
                .toUpperCase()
          ...
          killstreakActions.add(
            new SoundKillstreakAction(theSound, soundTarget, volume, pitch)
          );
        }
      }
      if (actionObj.has("effects")) {
        ...
        for (JsonElement jsonElem : actionObj.getAsJsonArray("effects")) {
          ...
          potionEffects.add(
            EffectDeserializer.parse(jsonElem.getAsJsonObject())
          );
        }
