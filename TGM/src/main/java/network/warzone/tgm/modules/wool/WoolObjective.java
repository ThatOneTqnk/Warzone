...
public class WoolObjective implements Listener {
  ...
  public void onPlace(BlockPlaceEvent event) {
    if (event.getBlockPlaced().getType() == block) {
      if (
        !completed &&
        podium.contains(event.getBlockPlaced().getLocation()) &&
        owner.containsPlayer(event.getPlayer())
      ...
      if (podium.contains(event.getBlockPlaced().getLocation())) {
        ...
        event
          ...
          .sendMessage(
            ...
            "You may only place " +
            ChatColor.YELLOW +
            ItemUtils.materialToString(block) +
            ChatColor.RED +
            " in the podium!"
          );
      }
  ...
  public void onWoolCraft(CraftItemEvent event) {
    if (event.getRecipe().getResult().getType().name().contains("WOOL")) { //TODO 1.13 Temp fix
      ...
      event
        .getWhoClicked()
        .sendMessage(ChatColor.RED + "You are not allowed to craft wool.");
    }
  }
  ...
  public void onPickup(EntityPickupItemEvent event) {
    if (
      ...
      event.getItem().getItemStack().getType() == block
    ) {
  ...
  public void onTeamChange(TeamChangeEvent event) {
    ...
    if (
      !touches.containsKey(event.getPlayerContext().getPlayer().getUniqueId())
    ) return;

    touches.remove(event.getPlayerContext().getPlayer().getUniqueId());

    for (WoolObjectiveService woolObjectiveService : services) {
      woolObjectiveService.drop(
