...
public class RegionManagerModule extends MatchModule {
  ...
  public void load(Match match) {
    ...
    if (match.getMapContainer().getMapInfo().getJsonObject().has("regions")) {
      for (JsonElement regionElement : match
        .getMapContainer()
        .getMapInfo()
        .getJsonObject()
  ...
  public Region getRegion(Match match, JsonElement jsonElement) {
    if (jsonElement.isJsonPrimitive()) {
      ...
      switch (type) {
        ...
        case "hemisphere":
          region =
            new HemisphereRegion(
              Parser.convertLocation(
                match.getWorld(),
                regionJson.get("center")
              ),
              regionJson.get("radius").getAsDouble(),
              regionJson.has("direction")
                ? HemisphereRegion.parseHemisphereDirection(
                  regionJson.get("direction")
                )
                : HemisphereRegion.HemisphereFace.NEGATIVE_Z
            );
          break;
        case "meta":
