...
public class DeathModule extends MatchModule implements Listener {
    ...
    if (event.getEntity() instanceof Player) {
      ...
      if (event instanceof EntityDamageByEntityEvent) {
        ...
        if (byEntityEvent.getDamager() instanceof Player) damager =
          (Player) byEntityEvent.getDamager(); else if (
        ...
        if (
          damager != null && teamManagerModule.getTeam(damager).isSpectator()
        ) return;

        deathInfo.killer = damager;
        deathInfo.item =
          determineItemFromDamager(byEntityEvent.getDamager(), damager);

        deathInfo.killerName = damager == null ? null : damager.getName();
        deathInfo.stampKill = damager == null ? -1 : System.currentTimeMillis();
        deathInfo.killerTeam =
          damager == null ? null : teamManagerModule.getTeam(damager);
        deathInfo.killerLocation =
          damager == null ? null : damager.getLocation();
      }

      if (
        p.getHealth() - event.getFinalDamage() <= 0 ||
  ...
  private void onDeath(Player player, DeathInfo deathInfo) {
    if (
      deathInfo.stampKill > 0 &&
      System.currentTimeMillis() - deathInfo.stampKill >= 1000 * 30
    ) deathInfo.killer = null;
    setDead(player);
    Bukkit
      .getPluginManager()
      .callEvent(
        new TGMPlayerDeathEvent(
          deathInfo.player,
    ...
    for (ItemStack stack : event.getDrops()) {
      if (stack != null) {
        event
          .getVictim()
          .getWorld()
          .dropItemNaturally(event.getDeathLocation(), stack);
      }
    }
  }
