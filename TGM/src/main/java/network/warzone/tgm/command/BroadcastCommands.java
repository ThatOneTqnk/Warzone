...
public class BroadcastCommands {
  ...
  @Command(
    ...
    desc = "Broadcast tool",
    usage = "(list|preset|raw|playerpreset|playerraw|config|start|stop|reload)",
    min = 1
  )
  ...
  public static void broadcast(CommandContext cmd, CommandSender sender) {
    ...
    if ("preset".equalsIgnoreCase(cmd.getString(0))) {
      if (cmd.argsLength() <= 1) {
        sender.sendMessage(
          ChatColor.RED + "/" + cmd.getCommand() + " preset <id> [args]"
        );
        return;
      }
      if (
        !broadcastManager.broadcast(
          cmd.getString(1),
          (cmd.argsLength() > 2 ? cmd.getParsedSlice(2) : new String[] {})
        )
      ) sender.sendMessage(ChatColor.RED + "Broadcast not found.");
    } else if ("raw".equalsIgnoreCase(cmd.getString(0))) {
      if (cmd.argsLength() <= 1) {
        sender.sendMessage(
          ChatColor.RED + "/" + cmd.getCommand() + " raw <message>"
        );
        return;
      }
      broadcastManager.broadcastRaw(cmd.getRemainingString(1));
    } else if (
      "ppreset".equalsIgnoreCase(cmd.getString(0)) ||
      "playerpreset".equalsIgnoreCase(cmd.getString(0))
    ) {
      if (cmd.argsLength() <= 1) {
        sender.sendMessage(
          ChatColor.RED +
      ...
      if (target == null) {
        sender.sendMessage(
          ChatColor.RED + "Player not found \"" + cmd.getString(1) + "\"."
        );
        return;
      }
      if (
        !broadcastManager.broadcast(
          target,
          cmd.getString(2),
          (cmd.argsLength() > 3 ? cmd.getParsedSlice(3) : new String[] {})
        )
      ) sender.sendMessage(ChatColor.RED + "Broadcast not found.");
    } else if (
      "praw".equalsIgnoreCase(cmd.getString(0)) ||
      "playerraw".equalsIgnoreCase(cmd.getString(0))
    ) {
      if (cmd.argsLength() <= 2) {
        sender.sendMessage(
          ChatColor.RED +
      ...
      if (cmd.argsLength() <= 1) {
        sender.sendMessage(
          ...
          "/" +
          cmd.getCommand() +
          " config <autobroadcast|interval|url|queue> [value]"
        );
        return;
      }
      if ("autobroadcast".equalsIgnoreCase(cmd.getString(1))) {
        if (
          cmd.argsLength() > 2 &&
          !broadcastManager.setAutobroadcast(
            Boolean.parseBoolean(cmd.getString(2))
          )
        ) {
        ...
        sender.sendMessage(
          ...
          "Autobroadcast: " +
          (
            broadcastManager.isAutoBroadcast() ? ChatColor.GREEN : ChatColor.RED
          ) +
          broadcastManager.isAutoBroadcast()
        );
      } else if ("interval".equalsIgnoreCase(cmd.getString(1))) {
        if (
          cmd.argsLength() > 2 &&
          !broadcastManager.setInterval(Integer.parseInt(cmd.getString(2)))
        ) {
        ...
        sender.sendMessage(
          ...
          "Interval: " +
          ChatColor.GREEN +
          broadcastManager.getInterval()
        );
      } else if ("url".equalsIgnoreCase(cmd.getString(1))) {
        if (
          cmd.argsLength() > 2 &&
          !broadcastManager.setURL(
            "-".equals(cmd.getString(2)) ? null : cmd.getString(2)
          )
        ) {
        ...
        sender.sendMessage(
          ...
          "Broadcasts URL: " +
          ChatColor.GREEN +
          broadcastManager.getUrl()
        );
      } else if ("queue".equalsIgnoreCase(cmd.getString(1))) {
        if (
          cmd.argsLength() > 2 &&
          !broadcastManager.setQueue(
            "-".equals(cmd.getString(2))
              ? Collections.emptyList()
              : Arrays.asList(cmd.getParsedSlice(2))
          )
        ) {
          sender.sendMessage(
            ChatColor.RED + "Could not set autobroadcast queue."
          );
          return;
        }
        sender.sendMessage(
      ...
      sender.sendMessage(
        ChatColor.GREEN + "Broadcast configuration & list reloaded."
      );
      ...
      if (broadcastManager.getTask() != null) {
        sender.sendMessage(
          ChatColor.RED + "Autobroadcast task already running."
        );
        return;
      }
