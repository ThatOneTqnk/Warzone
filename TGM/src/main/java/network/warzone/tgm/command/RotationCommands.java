...
public class RotationCommands {
  ...
  public static void viewRotations(
    CommandContext context,
    CommandSender sender
  ) throws CommandException {
    int index = context.argsLength() == 0 ? 1 : context.getInteger(0);
    List<Rotation> rotationLibrary = TGM
      .get()
      .getMatchManager()
      .getMapRotation()
      .getRotationLibrary();
    ...
    sender.sendMessage(
      ChatColor.YELLOW + "Rotations (" + index + "/" + pages + "): "
    );
    try {
  ...
  public static void setRotation(CommandContext context, CommandSender sender) {
    ...
    if (
      !TGM.get().getMatchManager().getMapRotation().hasRotation(rotationName)
    ) {
      sender.sendMessage(
        ChatColor.RED +
        "Invalid rotation. Try /rotations to view all rotations."
      );
      return;
    }
    ...
    sender.sendMessage(
      ChatColor.translateAlternateColorCodes(
        '&',
        "&aUpdated rotation to &e" + rotationName + "&a."
      )
    );
  }
  ...
  public static void rotation(final CommandContext cmd, CommandSender sender)
    ...
    try {
      for (int i = 0; i < pageSize; i++) {
        ...
        TextComponent message = CycleCommands.mapToTextComponent(
          position,
          map.getMapInfo()
        );
        sender.spigot().sendMessage(message);
  ...
  private static TextComponent rotationToTextComponent(
    int position,
    Rotation rotation
  ) {
    ...
    if (
      rotation.equals(
        TGM.get().getMatchManager().getMapRotation().getRotation()
      )
    ) {
      rotationName =
        ChatColor.GREEN + "" + (position + 1) + ". " + rotationName;
    } else {
      rotationName =
        ChatColor.WHITE + "" + (position + 1) + ". " + rotationName;
    }
    TextComponent message = new TextComponent(rotationName);
    message.setClickEvent(
      new ClickEvent(
        ClickEvent.Action.SUGGEST_COMMAND,
        "/setrot " + rotation.getName()
      )
    );
    ComponentBuilder builder = new ComponentBuilder(
      ChatColor.GOLD +
      rotation.getName() +
      (rotation.isDefault() ? ChatColor.GRAY + " - Default" : "")
    )
      .append("\n\n")
      .append(ChatColor.GRAY + "Requirements: ")
      .append(
        ChatColor.YELLOW +
        (
          rotation.isDefault()
            ? rotation.getRequirements().getMin() +
            "-" +
            rotation.getRequirements().getMax() +
            " players"
            : "N/A"
        )
      )
      .append("\n")
      .append(ChatColor.GRAY + "Map Count: ")
      .append(ChatColor.YELLOW + String.valueOf(rotation.getMaps().size()));
    message.setHoverEvent(
      new HoverEvent(HoverEvent.Action.SHOW_TEXT, builder.create())
    );
    return message;
  }
}
