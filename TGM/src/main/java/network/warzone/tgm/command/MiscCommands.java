...
public class MiscCommands {
  ...
  public static void ping(CommandContext cmd, CommandSender sender) {
    ...
    if (cmd.argsLength() > 0) {
      ...
      if (player == null) {
        sender.sendMessage(
          ChatColor.RED + "Player not found: " + cmd.getString(0)
        );
        return;
      }
      ...
      sender.sendMessage(
        ChatColor.RED +
        "As console, you can use /ping <player> to check someone's ping."
      );
      return;
    }
      ...
      (
        (playerPing >= 0)
          ? (
            ...
            ChatColor.GRAY +
            "'" +
            (player.getName().endsWith("s") ? "" : "s") +
            " ping is " +
            ChatColor.AQUA +
            playerPing +
            "ms"
          )
          : ChatColor.RED + "Could not get ping."
      );
  ...
  public static void tgm(CommandContext commandContext, CommandSender sender) {
    ...
    sender.sendMessage(
      String.format("%sUptime: %s%s", ChatColor.GRAY, ChatColor.WHITE, uptime)
    );
    sender.sendMessage(
      String.format(
        "%sMemory usage: (%s%s/%s%1$s):",
        ChatColor.GRAY,
        ChatColor.WHITE,
        ServerUtil.getFormattedUsedMemory(),
        ServerUtil.getFormattedTotalMemory()
      )
    );
    sender.sendMessage(
      String.format(
        "%sLoaded worlds (%s%d%1$s):",
        ChatColor.GRAY,
        ChatColor.WHITE,
    ...
    Bukkit
      ...
      .forEach(
        w ->
          sender.sendMessage(
            ChatColor.GRAY + " - " + ChatColor.WHITE + w.getName()
          )
      );
    ...
    sender.sendMessage(
      String.format(
        ...
        ChatColor.GRAY,
        ChatColor.WHITE,
        TGM.get().getMatchManager().getMatchNumber()
      )
    );
    sender.sendMessage(
      String.format(
        "%sPlayer contexts: %s%s%s",
        ChatColor.GRAY,
        ChatColor.WHITE,
        TGM.get().getPlayerManager().getPlayers().size(),
        TGM.get().getPlayerManager().getPlayers().size() !=
    ...
    sender.sendMessage(
      String.format(
        ...
        ChatColor.GRAY,
        ChatColor.WHITE,
        TGM.get().getMatchManager().getMatch().getModules().size()
      )
    );
    sender.sendMessage(
      String.format(
        "%sCached profiles: %s%s",
        ChatColor.GRAY,
        ChatColor.WHITE,
        ProfileCache.getInstance().size()
      )
    );
  }

  @Command(
    aliases = { "profilecache" },
    desc = "Manage the profile cache.",
    usage = "<clear>",
    min = 1
  )
  @CommandPermissions({ "tgm.command.profilecache" })
  public static void profileCache(
  ...
  public static void rules(
    ...
    sender.sendMessage(
      ChatColor.YELLOW +
      "Please read and abide by our rules which can be found here: " +
      TGM.get().getConfig().getString("server.rules")
    );
  }
}
