...
public class PunishCommands {
  ...
  @Command(
    aliases = { "punish", "pun", "pg", "pgui" },
    desc = "Punishment interface",
    usage = "(reload | <players>)",
    flags = "s"
  )
  public static void punish(CommandContext cmd, CommandSender sender)
    ...
    if (sender instanceof Player) {
      ...
      if (
        !player.hasPermission("tgm.punish.ban-ip") &&
      ...
      if (cmd.argsLength() >= 1) {
        ...
        PunishMenu.PunishConfig config = PunishMenu.getConfig(
          player.getUniqueId()
        );
        if (
          !cmd.hasFlag('s') || !player.hasPermission("tgm.punish.confirm.skip")
        ) {
          ItemStack configItem = config.toItem();
          String[] lore = new String[players.length + 2];
          lore[0] = "";
          lore[1] = ChatColor.GRAY + "Players:";
          for (int i = 0; i < players.length; i++) lore[i + 2] =
            ChatColor.GRAY + "- " + ChatColor.WHITE + players[i];
          ItemFactory.appendLore(configItem, lore);
          new ConfirmMenu(
            player,
            ChatColor.UNDERLINE + "Confirm bulk punish",
            configItem,
            (p, e) -> {
              Arrays
                .stream(players)
                .forEach(
                  username ->
                    PunishMenu.issuePunishment(username, player, config)
                );
              p.closeInventory();
              player.playSound(
                player.getLocation(),
        ...
        Arrays
          .stream(players)
          .forEach(
            username -> PunishMenu.issuePunishment(username, player, config)
          );
        player.playSound(
          player.getLocation(),
          Sound.BLOCK_NOTE_BLOCK_PLING,
  ...
  @Command(
    ...
    desc = "IP Ban a rulebreaker",
    min = 2,
    usage = "(name|ip) (length) (reason...)",
    anyFlags = true,
    flags = "s"
  )
  ...
  public static void banIP(CommandContext cmd, CommandSender sender) {
    ...
    if (timeUnitPair == null) {
      sender.sendMessage(
        ChatColor.RED + "Invalid duration. Should be: 1m, 1h, 1d, etc."
      );
      return;
    }

    String reason = cmd.argsLength() > 2
      ? cmd.getRemainingString(2)
    ...
    if (isIP(name)) {
      ...
      if ((target = Bukkit.getPlayer(name)) != null) {
        issuePunishment(
          "ban",
          name,
          target.getAddress().getHostString(),
          true,
          sender,
          "IP banned",
          timeUnitPair,
  ...
  @Command(
    aliases = "ban",
    desc = "Ban a rulebreaker",
    min = 2,
    usage = "(name) (length) (reason...)",
    anyFlags = true,
    flags = "s"
  )
  ...
  public static void kick(CommandContext cmd, CommandSender sender) {
    ...
    String reason = cmd.argsLength() > 1
      ? cmd.getRemainingString(1)
      : "Inappropriate Behavior";
    ...
    issuePunishment(
      ...
      sender,
      "kicked",
      new TimeUnitPair(1, ChronoUnit.MILLIS),
      reason,
      false,
      !cmd.hasFlag('s')
    );
  ...
  @Command(
    aliases = { "punishments", "p" },
    desc = "Get player punishments",
    min = 1,
    max = 1,
    usage = "(name|ip)"
  )
  ...
  public static void punishments(CommandContext cmd, CommandSender sender) {
    ...
    Bukkit
      ...
      .runTaskAsynchronously(
        TGM.get(),
        () -> {
          PunishmentsListResponse punishmentsListResponse = TGM
            .get()
            .getTeamClient()
            .getPunishments(
              new PunishmentsListRequest(
                !isIP(name) ? name : null,
                isIP(name) ? name : null
              )
            );
          if (punishmentsListResponse.isNotFound()) {
            ...
            sender.sendMessage(
              ChatColor.YELLOW + "Punishments for " + displayName
            );
              ...
              sender
                ...
                .sendMessage(
                  punishmentToTextComponent(
                    punishment,
                    map.get(punishment.getPunished()),
                    map.getOrDefault(punishment.getPunisher(), "Console"),
                    true
                  )
                );
            }
  ...
  public static void lookup(CommandContext cmd, CommandSender sender) {
    ...
    Bukkit
      ...
      .runTaskAsynchronously(
        TGM.get(),
        () -> {
          PlayerInfoResponse playerInfoResponse = TGM
            .get()
            .getTeamClient()
            .getPlayerInfo(new PlayerInfoRequest(filter, null));
          if (playerInfoResponse.isError()) {
            ...
            sender.sendMessage(
              ChatColor.YELLOW +
              "Info for " +
              playerInfoResponse.getQueryFilter() +
              ":"
            );
            ...
            sender.sendMessage(
              ...
              "First join: " +
              ChatColor.RESET +
              new Date(profile.getInitialJoinDate()).toString() +
              "\n" +
              ChatColor.GRAY +
              "Last online: " +
              ChatColor.RESET +
              new Date(profile.getLastOnlineDate()).toString()
            );
          }
        }
      );
      ...
      .runTaskAsynchronously(
        ...
        () -> {
          PlayerInfoResponse playerInfoResponse = TGM
            .get()
            .getTeamClient()
            .getPlayerInfo(new PlayerInfoRequest(null, filter));
          if (playerInfoResponse.isError()) {
            sender.sendMessage(ChatColor.RED + playerInfoResponse.getMessage());
          } else {
            sender.sendMessage(
            ...
            sender.sendMessage(
              ...
              ChatColor.RESET +
              "(" +
              playerInfoResponse.getUsers().size() +
              ")" +
              ChatColor.GRAY +
              ":"
            );
              ...
              sender
                ...
                .sendMessage(
                  new ComponentBuilder(
                    ChatColor.GRAY + " - " + ChatColor.RESET + profile.getName()
                  )
                    .event(
                      new HoverEvent(
  ...
  public static void alts(CommandContext cmd, CommandSender sender) {
    Bukkit
      ...
      .runTaskAsynchronously(
        TGM.get(),
        () -> {
          PlayerAltsResponse response = TGM
            .get()
            .getTeamClient()
            .getAlts(cmd.getString(0));
          if (response == null) {
            ...
            if (response.getUsers().isEmpty()) {
              sender.sendMessage(
                ChatColor.GREEN +
                response.getLookupUser().getName() +
                " has no known alts."
              );
              return;
            }
            ...
            for (UserProfile user : response.getUsers()) {
              ...
              PunishmentsListResponse punishmentsListResponse = TGM
                ...
                .getTeamClient()
                .getPunishments(
                  new PunishmentsListRequest(user.getName(), null)
                );
              if (!punishmentsListResponse.isNotFound()) {
                for (Punishment punishment : punishmentsListResponse.getPunishments()) {
                  if (
                    "BAN".equals(punishment.getType().toUpperCase()) &&
                    punishment.isActive()
                  ) {
                    isBanned = true;
                    break;
                  ...
                  if (
                    "MUTE".equals(punishment.getType().toUpperCase()) &&
                    punishment.isActive()
                  ) {
                    isMuted = true;
                  }
  ...
  public static void revert(CommandContext cmd, CommandSender sender) {
    ...
    Bukkit
      ...
      .runTaskAsynchronously(
        TGM.get(),
        () -> {
          RevertPunishmentResponse revertPunishmentResponse = TGM
            .get()
            .getTeamClient()
            .revertPunishment(id);
          if (
            revertPunishmentResponse == null ||
            revertPunishmentResponse.isNotFound()
          ) {
            ...
            if (revertPunishmentResponse.isSuccess()) {
              Bukkit
                .getOnlinePlayers()
                .stream()
                .filter(player -> player.hasPermission("tgm.punish.revert"))
                .forEach(
                  player ->
                    player
                      .spigot()
                      .sendMessage(
                        new TextComponent(
                          ChatColor.YELLOW + sender.getName() + " reverted "
                        ),
                        punishmentToTextComponent(
                          revertPunishmentResponse.getPunishment(),
                          userMappings.get(
                            revertPunishmentResponse
                              .getPunishment()
                              .getPunished()
                          ),
                          userMappings.get(
                      ...
                      (
                        punishment =
                          playerContext
                            .getUserProfile()
                            .getPunishment(
                              revertPunishmentResponse.getPunishment().getId()
                            )
                      ) !=
                      null
                    ) {
              ...
              sender.sendMessage(
                ChatColor.RED + "Punishment was already reverted."
              );
            }
          }
        }
    ...
    if (action.equalsIgnoreCase("mute")) {
      if (TGM.get().getConfig().getBoolean("chat.enabled")) {
        ...
        Bukkit.broadcastMessage(
          ChatColor.DARK_AQUA + sender.getName() + " muted the chat."
        );
      } else {
        TGM.get().getConfig().set("chat.enabled", true);
        TGM.get().saveConfig();
        Bukkit.broadcastMessage(
          ChatColor.DARK_AQUA + sender.getName() + " unmuted the chat."
        );
      }
      ...
      Bukkit.broadcastMessage(
        ChatColor.DARK_AQUA + sender.getName() + " cleared the chat."
      );
    }
  }
  ...
  public static void report(CommandContext cmd, CommandSender sender) {
    ...
    if (ReportsModule.cooldown(reporter.getUniqueId().toString())) {
      reporter.sendMessage(
        ChatColor.RED + "Please wait until reporting again!"
      );
      return;
    }
    ...
    ReportsModule.setCooldown(
      reporter.getUniqueId().toString(),
      report.getTimestamp()
    );

    PlayerContext reportedContext = TGM
      .get()
      .getPlayerManager()
      .getPlayerContext(reported.getUniqueId());
    ...
    if (reportedContext.isNicked()) {
      reportedNameBuilder
        .append(" ")
        .append("&8(&a")
        .append(reportedContext.getOriginalName())
        .append("&8)");
    }
      ...
      if (player.hasPermission("tgm.reports")) {
        TextComponent message = new TextComponent(
          ChatColor.translateAlternateColorCodes(
            ...
            reportedName +
            " &7for &r" +
            cmd.getJoinedStrings(1)
          )
        );
        message.setClickEvent(
          new ClickEvent(
            ClickEvent.Action.SUGGEST_COMMAND,
            "/tp " + report.getReported()
          )
        );
    ...
    reporter.sendMessage(
      ChatColor.GREEN + "Your report has been sent to online staff."
    );
  }
  ...
  public static void reports(CommandContext cmd, CommandSender sender)
    ...
    sender.sendMessage(
      ChatColor.YELLOW + "Reports (" + index + "/" + pages + "): "
    );
    try {
      for (int i = 0; i < pageSize; i++) {
        ...
        String reported = ChatColor.translateAlternateColorCodes(
          '&',
          "&8[" +
          report.getAmount() +
          "] &5" +
        ...
        message.setHoverEvent(
          new HoverEvent(
            HoverEvent.Action.SHOW_TEXT,
            new ComponentBuilder(ChatColor.GOLD + reported)
              .append("\n\n")
              .append(ChatColor.GRAY + "Date/Time: ")
              .append(dateFormat.format(date))
              .append("\n")
              .append(ChatColor.GRAY + "Reporter: ")
              .append(report.getReporter())
              .create()
          )
        );
  ...
  private static void issuePunishment(
    String type,
    String name,
    CommandSender punisher,
    String verb,
    TimeUnitPair timeUnitPair,
    String reason,
    boolean time,
    boolean broadcast
  ) {
  ...
  private static void issuePunishment(
    ...
    Bukkit
      ...
      .runTaskAsynchronously(
        TGM.get(),
        () -> {
          IssuePunishmentResponse response = TGM
            .get()
            .getTeamClient()
            .issuePunishment(
              new IssuePunishmentRequest(
                ...
                ip,
                ip_ban,
                punisher instanceof Player
                  ? ((Player) punisher).getUniqueId()
                  : null,
                type.toUpperCase(),
                ...
                () -> {
                  ...
                  broadcastPunishment(
                    ...
                    TGM
                      .get()
                      .getNickManager()
                      .getOriginalName(punisher.getName())
                  ...
                  if (
                    ...
                    (target = Bukkit.getPlayer(response.getName())) != null
                  ) {
                    TGM
                      .get()
                      .getPlayerManager()
                      .getPlayerContext(target)
                      .getUserProfile()
                      .addPunishment(response.getPunishment());
                  }
                }
              );
          }
  ...
  private static boolean kickPlayer(Punishment punishment, String name) {
    if (punishment.getType().equalsIgnoreCase("ban")) {
      String reason =
        ...
        ChatColor.RESET +
        ChatColor.translateAlternateColorCodes('&', punishment.getReason()) +
        "\n\n" +
        ChatColor.RED +
        "Ban expires: " +
        ChatColor.RESET +
        (
          punishment.getExpires() != -1
            ? new Date(punishment.getExpires()).toString()
            : "Never"
        ) +
        "\n" +
        ChatColor.AQUA +
        "Appeal at " +
        TGM.get().getConfig().getString("server.appeal") +
        "\n" +
        ChatColor.GRAY +
        "ID: " +
      ...
      if (punishment.isIp_ban()) {
        ...
        for (Player player : Bukkit.getOnlinePlayers()) {
          if (
            Objects
              .requireNonNull(player.getAddress())
              .getHostString()
              .equals(punishment.getIp())
          ) {
  ...
  private static TextComponent punishmentToTextComponent(
    Punishment punishment,
    String punished,
    String punisher,
    boolean revertOption
  ) {
    SimpleDateFormat dateFormat = new SimpleDateFormat("yyyy/MM/dd HH:mm");
    TextComponent textComponent = new TextComponent(
      ChatColor.GRAY +
      "[" +
      dateFormat.format(new Date(punishment.getIssued())) +
      "] " +
      (
        punishment.isReverted()
          ? ChatColor.STRIKETHROUGH + ""
          : (punishment.isActive() ? ChatColor.RED : ChatColor.YELLOW)
      ) +
      punishment.getType() +
      ChatColor.RESET +
      " " +
      (punished == null ? punishment.getIp() : punished)
    );
      ...
      new HoverEvent(
        ...
        new BaseComponent[] {
          ...
          new TextComponent(
            ...
            "\nType: " +
            ChatColor.RESET +
            punishment.getType().toUpperCase()
          ),
          new TextComponent(
            ChatColor.GRAY +
            "\nPunished IP: " +
          ...
          new TextComponent(
            ...
            "\nIssued: " +
            ChatColor.RESET +
            new Date(punishment.getIssued()).toString()
          ),
          new TextComponent(
            ChatColor.GRAY +
            "\nExpires: " +
    ...
    if (revertOption && !punishment.isReverted()) textComponent.setClickEvent(
      new ClickEvent(
        ClickEvent.Action.SUGGEST_COMMAND,
        "/revert " + punishment.getId().toString()
      )
    );
  ...
  private static void broadcastPunishment(
    ...
    TGM
      ...
      .info(
        String.format(
          "new-punishment {name=%s, ip=%s, punisher=%s, action=%s, timeUnitPair=%s, reason=%s, timed=%b, public=%s}",
          name,
          ip,
          punisher,
          action,
          timeUnitPair.toString(),
          reason,
          timed,
          everyone
        )
    ...
    if (name != null) {
        ...
        if (everyone) { // Timed & Public
          for (Player p : Bukkit.getOnlinePlayers()) {
            if (p.hasPermission("tgm.punish.list")) {
              Players.sendMessage(
                p,
                "%s%s&7 has been &c%s&7 by %s%s&7 for %s%s&7 for &r%s",
                punishedColor,
                name,
                action,
                punisherColor,
              ...
              Players.sendMessage(
                p,
                "%s%s&7 has been &c%s&7 for %s%s&7 for &r%s",
                punishedColor,
                name,
                action,
                durationColor,
          ...
          for (Player p : Bukkit.getOnlinePlayers()) {
            if (p.hasPermission("tgm.punish.list")) {
              Players.sendMessage(
                p,
                "&7[SILENT] %s%s&7 has been &c%s&7 by %s%s&7 for %s%s&7 for &r%s",
                punishedColor,
                name,
                action,
                punisherColor,
        ...
        if (everyone) { // Non-timed & Public
          for (Player p : Bukkit.getOnlinePlayers()) {
            if (p.hasPermission("tgm.punish.list")) {
              Players.sendMessage(
                p,
                "%s%s&7 has been &c%s&7 by %s%s&7 for &r%s",
                punishedColor,
                name,
                action,
                punisherColor,
          ...
          for (Player p : Bukkit.getOnlinePlayers()) {
            if (p.hasPermission("tgm.punish.list")) {
              Players.sendMessage(
                p,
                "&7[SILENT] %s%s&7 has been &c%s&7 by %s%s&7 for &r%s",
                punishedColor,
                name,
                action,
                punisherColor,
      ...
      if (timed) { // Timed & Silent
        ...
        String result = String.format(
          "&7[SILENT] %s%s&7 has &c%s&7 %s%s&7 for &r%s",
          punisherColor,
          punisher,
          action,
          punishedColor,
