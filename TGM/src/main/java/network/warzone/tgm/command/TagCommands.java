  ...
  public static void tags(final CommandContext cmd, CommandSender sender) {
    if (cmd.argsLength() == 0) {
      if (!(sender instanceof Player)) {
        sender.sendMessage(
          ChatColor.RED + "Only players may open the tags GUI."
        );
        return;
      }
      ...
      TagsMenu tagsMenu = new TagsMenu(
        ChatColor.UNDERLINE + "Your tags",
        9 * 6,
        playerContext
      );
      ...
      if (cmd.getString(0).equalsIgnoreCase("set")) {
        if (cmd.argsLength() < 2) {
          if (sender.hasPermission("tgm.tags.set")) sender.sendMessage(
            ChatColor.RED +
            "Usage: /" +
            cmd.getCommand() +
            " set <tag> [player]"
          ); else sender.sendMessage(
            ChatColor.RED + "Usage: /" + cmd.getCommand() + " set <tag>"
          );
          return;
        }
        ...
        String tag;
        if ("-".equals(cmd.getString(1))) tag = null; else tag =
          cmd.getString(1);
        String target;
        if (
          sender.hasPermission("tgm.tags.set") && cmd.argsLength() >= 3
        ) target = cmd.getString(2); else target = sender.getName();
        Bukkit
          .getScheduler()
          .runTaskAsynchronously(
            TGM.get(),
            () -> {
              PlayerTagsUpdateResponse response = TGM
                .get()
                .getTeamClient()
                .updateTag(target, tag, PlayerTagsUpdateRequest.Action.SET);

              if (!response.isError()) {
                String finalTag = tag == null
                  ? ChatColor.GRAY + "" + ChatColor.ITALIC + "none"
                  : ChatColor.translateAlternateColorCodes(
                    '&',
                    response.getActiveTag()
                  );
                Player player;
                ...
                if (responsePlayer.length() <= 16) {
                  ...
                  player =
                    Bukkit.getPlayer(UUID.fromString(response.getPlayer()));
                }
                if (player != null) {
                  TGM
                    .get()
                  ...
                  if (
                    ...
                    ChatColor.GREEN + "Set your active tag to " + finalTag
                  ); else sender.sendMessage(
                    ChatColor.GREEN +
                    "Set " +
                    player.getName() +
          ...
          .runTaskAsynchronously(
            ...
            () -> {
              PlayerTagsUpdateResponse response = TGM
                .get()
                .getTeamClient()
                .updateTag(target, tag, PlayerTagsUpdateRequest.Action.ADD);
              if (!response.isError()) {
                Player player;
                String responsePlayer = response.getPlayer();
                if (responsePlayer.length() <= 16) {
          ...
          .runTaskAsynchronously(
            ...
            () -> {
              PlayerTagsUpdateResponse response = TGM
                .get()
                .getTeamClient()
                .updateTag(target, tag, PlayerTagsUpdateRequest.Action.REMOVE);
        ...
        Bukkit
          ...
          .runTaskAsynchronously(
            TGM.get(),
            () -> {
              GetPlayerByNameResponse playerByNameResponse = TGM
                .get()
                .getTeamClient()
                .player(target);
              ...
              if (profile.getTags() != null && !profile.getTags().isEmpty()) {
                sender.sendMessage(
                  ChatColor.BLUE +
                  playerByNameResponse.getUser().getName() +
                  "'s tags:"
                );
                for (String tag : profile.getTags()) {
                  if (tag.equals(profile.getActiveTag())) sender.sendMessage(
                    ChatColor.GREEN +
                    "- " +
                    ChatColor.GRAY +
                    ChatColor.translateAlternateColorCodes('&', tag)
        ...
        if (subcmds.isEmpty()) sender.sendMessage(
          ChatColor.RED + "Usage: /" + cmd.getCommand() + " [set]"
        ); else sender.sendMessage(
          ChatColor.RED +
          "Usage: /" +
          cmd.getCommand() +
