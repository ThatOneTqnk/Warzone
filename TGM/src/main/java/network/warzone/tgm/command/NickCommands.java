...
public class NickCommands {
  ...
  public static void nick(CommandContext cmd, CommandSender sender)
    throws CommandNumberFormatException, NoSuchFieldException, IllegalAccessException {
    ...
    if (cmd.argsLength() == 0) {
      sender.sendMessage(
        ChatColor.RED +
        "Invalid usage. /nick [player] <create/set/preview/apply/reset/status> ..."
      );
      return;
    }
    ...
    if (!subcommands.contains(subcommand)) {
      ...
      if (target != null) {
        if (cmd.argsLength() > 1) {
          ...
          sender.sendMessage(
            ChatColor.RED +
            "Invalid usage. /nick [player] <create/set/preview/apply/reset> ..."
          );
          return;
        }
      } else {
  ...
  public static void whois(CommandContext cmd, CommandSender sender) {
    if (cmd.argsLength() > 0) {
      ...
      Optional<Nick> optionalNick = TGM
        .get()
        .getNickManager()
        .getNicks()
        .stream()
      ...
      if (optionalNick.isPresent()) {
        sender.sendMessage(
          ...
          " is " +
          ChatColor.YELLOW +
          optionalNick.get().getOriginalName()
        );
      } else {
  ...
  private static void create(
    ...
    NickManager.NickDetails details = new NickManager.NickDetails(
      ...
      cmd.getInteger(
        index + 8,
        context.getUserProfile(true).getWool_destroys()
      ),
      "true".equals(cmd.getString(index + 9, null))
    );
    nickManager.create(context, details);
    send(
      "Created new nick. Use [/nick preview] to view and [/nick apply] to apply it.",
      ChatColor.GREEN,
      sender
    );
  }
  ...
  private static void set(
    ...
    if (!optionalNick.isPresent()) {
      send(
        ...
        " not have a nick. Use [/nick create] to create one.",
        ChatColor.RED,
        sender
      );
      return;
    ...
    if (cmd.argsLength() > index + 1) {
      ...
      if ("name".equals(setSubcommand) && cmd.argsLength() > index + 2) {
        ...
        "stats".equals(setSubcommand) && cmd.argsLength() > index + 2
      ) {
      ...
      sender.sendMessage(
        ChatColor.RED +
        "Invalid usage. /nick (name) set <name/skin/rank/stats/frozen/Username> ..."
      );
    }
  }
  ...
  private static void setIndividualStat(
    Player sender,
    PlayerContext context,
    NickManager nickManager,
    String stat
    ...
    if ("good".equals(stat)) {
      ...
      sender.sendMessage(
        ChatColor.RED + "Invalid stat preset. Try good, average, bad or new"
      );
      return;
    }
    ...
    if (cmd.argsLength() > index + 3) {
      ...
      switch (stat) {
        case "kills":
          nickManager.update(
            context,
            nick -> nick.getProfile().setKills(newValue)
          );
          break;
        case "deaths":
          nickManager.update(
            context,
            nick -> nick.getProfile().setDeaths(newValue)
          );
          break;
        case "wins":
          nickManager.update(
            context,
            nick -> nick.getProfile().setWins(newValue)
          );
          break;
        case "losses":
          nickManager.update(
            context,
            nick -> nick.getProfile().setLosses(newValue)
          );
          break;
        case "objectives":
          nickManager.update(
            context,
            nick -> nick.getProfile().setWool_destroys(newValue)
          );
          break;
        default:
          sender.sendMessage(
            ChatColor.RED +
            "Invalid stat name. Try kills, deaths, wins, losses or objectives."
          );
          return;
      }
      sender.sendMessage(
    ...
    nickManager.update(
      ...
      nick -> {
        ...
        sender.sendMessage(
          ...
          "Toggled stat freeze " +
          ChatColor.YELLOW +
          (nick.getProfile().isFrozen() ? "on" : "off")
        );
      }
    );
  }
  ...
  private static void setName(
    ...
    if (cmd.hasFlag('f')) {
      ...
      sender.sendMessage(
        ChatColor.GREEN +
        "Force updated nickname to " +
        ChatColor.YELLOW +
        newName
      );
    } else {
      sender.sendMessage(
        ChatColor.GREEN + "Updated nickname to " + ChatColor.YELLOW + newName
      );
    }
  }
  ...
  private static void setSkin(
    ...
    if (cmd.hasFlag('f')) {
      ...
      sender.sendMessage(
        ChatColor.GREEN + "Force updated skin to " + ChatColor.YELLOW + newSkin
      );
    } else {
      sender.sendMessage(
        ChatColor.GREEN + "Updated skin to " + ChatColor.YELLOW + newSkin
      );
    }
  }
  ...
  private static void preview(
    ...
    sender.sendMessage(
      ...
      ChatColor.DARK_AQUA +
      (context.getPlayer().getName().endsWith("s") ? "' nick" : "'s nick")
    );
    sender.sendMessage("");
    sender.sendMessage(
      ChatColor.DARK_AQUA + "   Name: " + ChatColor.AQUA + nick.getName()
    );
    sender.sendMessage(
      ChatColor.DARK_AQUA +
      "   Skin: " +
      ChatColor.AQUA +
      nick.getDetails().getSkin()
    );
    if (nick.getProfile().getHighestRank() != null) sender.sendMessage(
      ChatColor.DARK_AQUA +
      "   Rank: " +
      ChatColor.AQUA +
      ChatColor.translateAlternateColorCodes(
        '&',
        nick.getProfile().getHighestRank().getPrefix()
      )
    );
    sender.sendMessage("");
    sender.sendMessage(
      ChatColor.DARK_AQUA + "   Level: " + ChatColor.AQUA + profile.getLevel()
    );
    sender.sendMessage(
      ChatColor.DARK_AQUA +
      "   XP: " +
  ...
  private static void apply(
    ...
    if (cmd.hasFlag('f')) {
      ...
      sender.sendMessage(
        ChatColor.GREEN + "Nick applied. Reconnect for it to take affect."
      );
    }
  }
  ...
  private static void reset(
    ...
    if (!nickManager.isNicked(context)) {
      send(
        ...
        " not have an active nick. Use [/nick create] to create one.",
        ChatColor.RED,
        sender
      );
      return;
  ...
  private static void status(
    ...
    sender.sendMessage(
      nick.isApplied()
        ? ChatColor.GREEN + "Applied"
        : ChatColor.RED + "Unapplied"
    );
    sender.sendMessage(
      nick.isActive() ? ChatColor.GREEN + "Active" : ChatColor.RED + "Inactive"
    );
    if (nick.getProfile().isFrozen()) sender.sendMessage(
      ChatColor.YELLOW + "Frozen"
    );
  }
