...
public class CycleCommands {
  ...
  @Command(
    aliases = { "maps" },
    desc = "View the maps that are on server, although not necessarily in the rotation.",
    usage = "[type]? [page]"
  )
  public static void maps(CommandContext cmd, CommandSender sender)
    ...
    List<MapContainer> mapLibrary = TGM
      .get()
      .getMatchManager()
      .getMapLibrary()
      .getMaps();
    ...
    if (type != null) {
      ...
      mapLibrary =
        mapLibrary
          .stream()
          .filter(map -> map.getMapInfo().getGametype().equals(finalType))
    ...
    sender.sendMessage(
      ChatColor.YELLOW + "Maps (" + index + "/" + pages + "): "
    );
    try {
  ...
  @Command(
    aliases = { "findmaps" },
    desc = "Find the maps that are on the server, although not necessarily in the rotation.",
    min = 1,
    usage = "<map name> [page]"
  )
  public static void findmaps(CommandContext cmd, CommandSender sender)
    throws CommandException {
    List<MapContainer> mapLibrary = TGM
      .get()
      .getMatchManager()
    ...
    for (int i = 0; i < mapLibrary.size(); i++) {
      if (
        cmd
          .getString(0)
          .equalsIgnoreCase(mapLibrary.get(i).getMapInfo().getName())
      ) { // Map with the exact same as input should always be listed first
        foundMaps.add(0, i);
      } else if (
        mapLibrary
          .get(i)
          ...
          .getName()
          .toLowerCase()
          .startsWith(cmd.getString(0).toLowerCase())
      ) {
        foundMaps.add(i);
      }
    }
    ...
    if (foundMaps.size() == 0) {
      sender.sendMessage(
        ChatColor.translateAlternateColorCodes(
          '&',
          "&cNo maps with the name &4" + cmd.getString(0) + "&c were found."
        )
      );
      return;
    }
    ...
    sender.sendMessage(
      ChatColor.YELLOW + "Found Maps (" + index + "/" + pages + "): "
    );
    try {
      for (int i = 0; i < pageSize; i++) {
        int position = pageSize * (index - 1) + i;
    ...
    if (matchStatus != MatchStatus.MID) {
      ...
      if (cmd.argsLength() > 0) {
        try {
          ...
          sender.sendMessage(
            ChatColor.RED + "Unknown time \"" + cmd.getString(0) + "\""
          );
          return;
        }
      }
  ...
  public static void start(CommandContext cmd, CommandSender sender) {
    ...
    if (matchStatus == MatchStatus.PRE) {
      ...
      if (!soloStart) sender.sendMessage(
        ChatColor.GREEN +
        "Match will start in " +
        time +
        " second" +
      ...
      sender.sendMessage(
        ChatColor.RED + "The match cannot be started at this time."
      );
    }
  }
  ...
  public static void end(CommandContext cmd, CommandSender sender) {
    ...
    if (matchStatus == MatchStatus.MID) {
      if (cmd.argsLength() > 0) {
        MatchTeam matchTeam = TGM
          .get()
          .getModule(TeamManagerModule.class)
          .getTeamFromInput(cmd.getJoinedStrings(0));
        if (matchTeam == null) {
          sender.sendMessage(
            ChatColor.RED +
            "Unable to find team \"" +
            cmd.getJoinedStrings(0) +
            "\""
          );
          return;
        }
  ...
  public static void setNext(CommandContext cmd, CommandSender sender) {
    if (cmd.argsLength() > 0) {
      ...
      for (MapContainer mapContainer : TGM
        ...
        if (
          mapContainer
            .getMapInfo()
            .getName()
            .equalsIgnoreCase(cmd.getJoinedStrings(0))
        ) {
          found = mapContainer;
        }
      }
        ...
        for (MapContainer mapContainer : TGM
          ...
          if (
            mapContainer
              ...
              .getName()
              .toLowerCase()
              .startsWith(cmd.getJoinedStrings(0).toLowerCase())
          ) {
            found = mapContainer;
          }
        }
      ...
      if (found == null) {
        sender.sendMessage(
          ChatColor.RED + "Map not found \"" + cmd.getJoinedStrings(0) + "\""
        );
        return;
      }
      ...
      sender.sendMessage(
        ...
        "Set the next map to " +
        ChatColor.YELLOW +
        found.getMapInfo().getName() +
        ChatColor.GRAY +
        " (" +
        found.getMapInfo().getVersion() +
        ")"
      );
    } else {
  ...
  public static void classCommand(CommandContext cmd, CommandSender sender) {
    ...
    if (
      TGM.get().getMatchManager().getMatch().getMatchStatus() ==
      MatchStatus.POST
    ) {
      sender.sendMessage(
        ChatColor.RED + "You cannot change classes at this time!"
      );
      return;
    }

    String chosenClassString = Strings.getTechnicalName(cmd.getString(0));
    GameClassModule gameClassModule = TGM
      .get()
      .getModule(GameClassModule.class);
    ...
    for (GameClassModule.GameClassStore gameClassStore : GameClassModule.GameClassStore.values()) {
      if (
        gameClassStore.name().equals(chosenClassString) &&
        gameClassModule.classSetHasInstance(gameClassStore.getHostGameClass())
      ) {
    ...
    if (
      Strings
        .getTechnicalName(gameClassModule.getCurrentClass(player))
        .equals(chosenClassString)
    ) {
    ...
    if (
      TGM.get().getMatchManager().getMatch().getMatchStatus() != MatchStatus.MID
    ) {
  ...
  public static void join(CommandContext cmd, CommandSender sender) {
    ...
    TeamManagerModule teamManager = TGM
      .get()
      .getModule(TeamManagerModule.class);
    PlayerContext playerContext = TGM
      .get()
      .getPlayerManager()
      .getPlayerContext((Player) sender);
    ...
    if (cmd.argsLength() > 0) {
      ...
      if (team == null) {
        sender.sendMessage(
          ChatColor.RED + "Unknown team: " + cmd.getRemainingString(0)
        );
        return;
      }
    } else {
    ...
    PlayerJoinTeamAttemptEvent event = new PlayerJoinTeamAttemptEvent(
      playerContext,
      oldTeam,
      team,
      autoJoin,
  ...
  @Command(
    ...
    max = 1,
    usage = "[player]",
    desc = "See your current killstreak"
  )
  public static void killstreak(CommandContext cmd, CommandSender sender) {
    ...
    if (cmd.argsLength() == 1) {
      ...
      killstreak =
        TGM
          .get()
          .getModule(KillstreakModule.class)
          .getKillstreak(player.getUniqueId().toString());

      if (killstreak == 0) {
        sender.sendMessage(
          ChatColor.translateAlternateColorCodes(
            '&',
            "&4" + player.getName() + " &cisn't on a kill streak."
          )
        );
        return;
      }
    ...
    if (matchTeam != null) {
      if (killstreak < 1 || matchTeam.isSpectator()) {
        ...
        sender.sendMessage(
          ChatColor.translateAlternateColorCodes(
            '&',
            (otherPlayer ? "&2" + player.getName() + " &ais " : "&aYou're ") +
            "on a kill streak of &2" +
            killstreak +
            "&a kill" +
            (killstreak == 1 ? "" : "s") +
            "."
      ...
      sender.sendMessage(
        ChatColor.RED + "Something went wrong. Try again later."
      );
    }
  }
  ...
  public static void teleport(CommandContext cmd, CommandSender sender) {
    ...
    if (matchTeam != null) {
      if (
        ...
        player.hasPermission("tgm.teleport")
      ) { // allow staff to tp outside of spectator
        if (cmd.argsLength() == 1) {
        ...
        player.sendMessage(
          ChatColor.RED + "You can only execute this command as a spectator!"
        );
      }
    } else {
      player.sendMessage(
      ...
      if (cmd.getString(0).equalsIgnoreCase("alias")) {
        if (cmd.argsLength() == 3) {
          MatchTeam matchTeam = TGM
            .get()
            .getModule(TeamManagerModule.class)
            .getTeamFromInput(cmd.getString(1));
          if (matchTeam == null) {
            sender.sendMessage(
              ChatColor.RED + "Unknown team \"" + cmd.getString(1) + "\""
            );
            return;
          }
          sender.sendMessage(
        ...
        if (cmd.argsLength() == 3) {
          MatchTeam matchTeam = TGM
            .get()
            .getModule(TeamManagerModule.class)
            .getTeamFromInput(cmd.getString(2));
          if (matchTeam == null) {
            sender.sendMessage(
              ChatColor.RED + "Unknown team \"" + cmd.getString(2) + "\""
            );
            return;
          }
          ...
          if (player == null) {
            sender.sendMessage(
              ChatColor.RED + "Unknown player \"" + cmd.getString(1) + "\""
            );
            return;
          }
  ...
  @Command(
    aliases = { "channel", "chatchannel", "cc" },
    desc = "Change or select a chat channel.",
    usage = "(all|team|staff)",
    min = 1
  )
  public static void channel(CommandContext cmd, CommandSender sender) {
    ...
    if (cmd.argsLength() == 0) {
      player.sendMessage(
        ColorConverter.filterString("&cUsage: /channel (name)")
      );
      return;
    }
    ...
    if (channel == null) {
      player.sendMessage(
        ColorConverter.filterString("&cInvalid channel: " + channelName)
      );
      player.sendMessage(
        ColorConverter.filterString(
          "&cChannels: ( " +
          StringUtils.join(
            Arrays
              .stream(ChatChannel.values())
    ...
    if (!channel.hasPermission(player)) {
      player.sendMessage(
        ColorConverter.filterString("&cError: Insufficient permissions.")
      );
      return;
    }
    ...
    player.sendMessage(
      ColorConverter.filterString(
        "&7You've been added to the channel &c&l" + channel.name() + "&7."
      )
    );
  }
  ...
  public static void t(CommandContext cmd, CommandSender sender) {
    ...
    if (cmd.argsLength() > 0) {
      ...
      TGM
        ...
        .sendTeamChat(playerContext, cmd.getJoinedStrings(0));
    }
  }
  ...
  public static void time(CommandContext cmd, CommandSender sender) {
    if (cmd.argsLength() <= 0) {
      ...
      sender.sendMessage(
        ...
        timeColor +
        Strings.formatTime(
          TGM.get().getModule(TimeModule.class).getTimeElapsed()
        )
      );
      return;
    }
    if (cmd.getString(0).equalsIgnoreCase("limit")) {
      ...
      if (cmd.argsLength() == 1 || cmd.argsLength() > 2) {
        sender.sendMessage(
          ChatColor.RED + "/" + cmd.getCommand() + " limit <seconds>"
        );
        return;
      }
      ...
      if (
        cmd.getString(1).equalsIgnoreCase("on") ||
        cmd.getString(1).equalsIgnoreCase("true")
      ) {
        timeModule.setTimeLimited(true);
        sender.sendMessage(
          ChatColor.AQUA + "Time limit: " + ChatColor.GREEN + "true"
        );
        return;
      } else if (
        cmd.getString(1).equalsIgnoreCase("off") ||
        cmd.getString(1).equalsIgnoreCase("false")
      ) {
        timeModule.setTimeLimited(false);
        sender.sendMessage(
          ChatColor.AQUA + "Time limit: " + ChatColor.RED + "false"
        );
        return;
      }
  ...
  public static void config(CommandContext cmd, CommandSender sender) {
    if (cmd.getString(0).equalsIgnoreCase("stats")) {
      if (cmd.argsLength() != 2) {
        sender.sendMessage(
          ChatColor.WHITE +
          "Stat uploading is set to \"" +
          TGM.get().getConfig().getBoolean("api.stats.enabled") +
          "\""
        );
        return;
      }
  ...
  @Command(
    ...
    min = 1,
    max = 1,
    desc = "List the top 10 players on the server"
  )
  public static void leaderboard(CommandContext cmd, CommandSender sender) {
    if (
      !TGM.get().getConfig().getBoolean("api.stats.enabled") ||
      !TGM.get().getConfig().getBoolean("api.enabled")
    ) {
      ...
      try {
        criterion =
          LeaderboardCriterion.valueOf(
            Strings.getTechnicalName(cmd.getString(0))
          );
      } catch (IllegalArgumentException e) {
        List<String> criteria = Arrays
          .stream(LeaderboardCriterion.values())
      ...
      Bukkit
        ...
        .runTaskAsynchronously(
          ...
          () -> {
            ...
            for (UserProfile player : TGM
              .get()
              .getTeamClient()
              .getLeaderboard(criterion)) {
              sender.sendMessage(
                profileToTextComponent(player, ++place, criterion).getText()
              );
            }
          }
        );
  ...
  @Command(
    ...
    desc = "Manage custom countdowns",
    usage = "<list|start|create|edit|cancel>",
    min = 1
  )
  @CommandPermissions({ "tgm.countdown" })
  public static void countdown(CommandContext cmd, CommandSender sender)
    throws CommandNumberFormatException {
    CountdownManagerModule countdownManagerModule = TGM
      .get()
      .getModule(CountdownManagerModule.class);
    if (cmd.getString(0).equalsIgnoreCase("list")) {
      ...
      if (countdowns.size() > 0) {
        ...
        sender.sendMessage(
          ChatColor.RED + "There are no registered countdowns."
        );
      }
    } else if (cmd.getString(0).equalsIgnoreCase("start")) {
      if (cmd.argsLength() <= 1) {
        sender.sendMessage(
          ChatColor.RED + "Usage: /" + cmd.getCommand() + " start (id)"
        );
        return;
      }
      CustomCountdown countdown = countdownManagerModule.getCountdown(
        cmd.getString(1)
      );
      if (countdown == null) {
      ...
      if (cmd.argsLength() <= 2) countdown.start(); else countdown.start(
        cmd.getInteger(2)
      );
      if (sender instanceof Player) sender.sendMessage(
        ChatColor.GREEN + "Countdown started."
      );
      ...
      if (cmd.argsLength() <= 3) {
        sender.sendMessage(
          ...
          "Usage: /" +
          cmd.getCommand() +
          " create <id> <time> <title> [color] [style] [visible] [invert] [teams] [onFinish]"
        );
        return;
      }
      ...
      BarColor color = cmd.argsLength() > 4
        ? BarColor.valueOf(Strings.getTechnicalName(cmd.getString(4)))
        : BarColor.PURPLE;
      BarStyle style = cmd.argsLength() > 5
        ? BarStyle.valueOf(Strings.getTechnicalName(cmd.getString(5)))
        : BarStyle.SOLID;
      boolean visible =
        cmd.argsLength() <= 6 || Boolean.parseBoolean(cmd.getString(6));
      boolean invert =
        cmd.argsLength() > 7 && Boolean.parseBoolean(cmd.getString(7));
      List<MatchTeam> teams = cmd.argsLength() > 8
        ? Arrays
          .stream(cmd.getString(8).split(","))
          .map(t -> TGM.get().getModule(TeamManagerModule.class).getTeamById(t))
          .collect(Collectors.toList())
        : new ArrayList<>();
      List<String> onFinish = cmd.argsLength() > 9
        ? Arrays.asList(cmd.getString(9).split(","))
        : new ArrayList<>();
      countdownManagerModule.addCountdown(
        id,
        new CustomCountdown(
          time,
          title,
      ...
      if (cmd.argsLength() < 3) {
        sender.sendMessage(
          ...
          "Usage: /" +
          cmd.getCommand() +
          " edit <id> <time|title|color|style|visible|invert|teams|onFinish> <value>"
        );
        return;
      }
      String id = cmd.getString(1);
      ...
      switch (cmd.getString(2)) {
        ...
        case "color":
          countdown.setColor(
            BarColor.valueOf(
              Strings.getTechnicalName(cmd.getRemainingString(3))
            )
          );
          break;
        case "style":
        ...
        case "teams":
          ...
          TeamManagerModule teamManagerModule = TGM
            .get()
            .getModule(TeamManagerModule.class);
          countdown.setTeams(
            Arrays
              .stream(cmd.getRemainingString(3).split(";"))
              .map(teamManagerModule::getTeamFromInput)
              .collect(Collectors.toList())
          );
          break;
        case "onFinish":
          ...
          countdown.setOnFinish(
            Arrays.asList(cmd.getRemainingString(3).split(";"))
          );
          break;
        default:
          sender.sendMessage(
      ...
      if (cmd.argsLength() < 2) {
        sender.sendMessage(
          ChatColor.RED + "Usage: /" + cmd.getCommand() + " cancel <id>"
        );
        return;
      }
      String id = cmd.getString(1);
  ...
  private static void viewStats(CommandSender sender, String target) {
    ...
    if (targetPlayer == null) {
      Bukkit
        ...
        .runTaskAsynchronously(
          TGM.get(),
          () -> {
            GetPlayerByNameResponse response = TGM
              .get()
              .getTeamClient()
              .player(target);
            ...
            sender.sendMessage("");
            sender.sendMessage(
              ChatColor.DARK_AQUA + "   Level: " + up.getLevel()
            );
            sender.sendMessage(
              ChatColor.DARK_AQUA +
              "   XP: " +
              ...
              "/" +
              ChatColor.DARK_AQUA +
              UserProfile.getRequiredXP(up.getLevel() + 1) +
              " (approx.)"
            );
            sender.sendMessage("");
            sender.sendMessage(
            ...
            sender.sendMessage(
              ChatColor.DARK_AQUA + "   K/D: " + ChatColor.AQUA + up.getKDR()
            );
            sender.sendMessage("");
            sender.sendMessage(
              ChatColor.DARK_AQUA + "   Wins: " + ChatColor.GREEN + up.getWins()
            );
            sender.sendMessage(
              ChatColor.DARK_AQUA +
              "   Losses: " +
            ...
            sender.sendMessage(
              ChatColor.DARK_AQUA + "   W/L: " + ChatColor.AQUA + up.getWLR()
            );
            sender.sendMessage(
              ChatColor.BLUE +
              ChatColor.STRIKETHROUGH.toString() +
    ...
    PlayerContext targetUser = TGM
      .get()
      .getPlayerManager()
      .getPlayerContext(targetPlayer);
    boolean getNickedStats =
      !target.equalsIgnoreCase(targetUser.getOriginalName()) &&
      targetUser.isNicked();
    UserProfile profile = getNickedStats
      ? targetUser.getUserProfile()
      : targetUser.getUserProfile(true);
    String levelString = getNickedStats
      ? targetUser.getLevelString()
      : targetUser.getLevelString(true);
    sender.sendMessage(
      ChatColor.BLUE +
      ChatColor.STRIKETHROUGH.toString() +
      "-------------------------------"
    ...
    sender.sendMessage(
      ...
      "/" +
      ChatColor.DARK_AQUA +
      UserProfile.getRequiredXP(profile.getLevel() + 1) +
      " (approx.)"
    );
    sender.sendMessage("");
    sender.sendMessage(
      ChatColor.DARK_AQUA + "   Kills: " + ChatColor.GREEN + profile.getKills()
    );
    sender.sendMessage(
      ChatColor.DARK_AQUA + "   Deaths: " + ChatColor.RED + profile.getDeaths()
    );
    sender.sendMessage(
      ChatColor.DARK_AQUA + "   K/D: " + ChatColor.AQUA + profile.getKDR()
    );
    sender.sendMessage("");
    sender.sendMessage(
      ChatColor.DARK_AQUA + "   Wins: " + ChatColor.GREEN + profile.getWins()
    );
    sender.sendMessage(
      ChatColor.DARK_AQUA + "   Losses: " + ChatColor.RED + profile.getLosses()
    );
    sender.sendMessage(
      ChatColor.DARK_AQUA + "   W/L: " + ChatColor.AQUA + profile.getWLR()
    );
    sender.sendMessage(
      ChatColor.BLUE +
      ChatColor.STRIKETHROUGH.toString() +
  ...
  public static void attemptJoinTeam(
    Player player,
    MatchTeam matchTeam,
    boolean autoJoin
  ) {
  ...
  public static void attemptJoinTeam(
    ...
    if (matchTeam.getMembers().size() >= matchTeam.getMax() && !ignoreFull) {
      player.sendMessage(
        ChatColor.RED + "Team is full! Wait for a spot to open up."
      );
      return;
    }
    ...
    if (!autoJoin) {
      if (!player.hasPermission("tgm.pickteam")) {
        player.sendMessage(
          ...
          "Only premium users can pick their team!\nPurchase a rank at " +
          TGM.get().getConfig().getString("server.store")
        );
        return;
      } else if (
        !player.hasPermission("tgm.pickteam.bypass") &&
        (
          !TGM.get().getApiManager().isStatsDisabled() ||
          !TGM
            .get()
            .getConfig()
            .getBoolean("map.team-picking-conditions.ignore-untracked")
        )
      ) {
        if (
          getPlayerCount() <
          TGM
            .get()
            .getConfig()
            .getInt("map.team-picking-conditions.min-players")
        ) {
          player.sendMessage(
            ChatColor.RED +
            "There are not enough players in the server for you to be able to pick your team."
          );
          return;
        } else if (
          !enoughPlayers(
            TGM
              .get()
              .getConfig()
              .getInt("map.team-picking-conditions.min-playing")
          )
        ) {
          player.sendMessage(
            ChatColor.RED +
            "There are not enough players already playing for you to be able to pick your team."
          );
          return;
        }
      }
  ...
  public static TextComponent profileToTextComponent(
    UserProfile profile,
    int place,
    LeaderboardCriterion criterion
  ) {
    ...
    main.setHoverEvent(
      new HoverEvent(
        ...
        new TextComponent[] {
          new TextComponent(
            ChatColor.AQUA + "Level: " + ChatColor.RESET + profile.getLevel()
          ),
          new TextComponent("\n"),
          new TextComponent(
    ...
    main.setClickEvent(
      new ClickEvent(
        ...
        "/stats " + profile.getName()
      )
    );
    return main;
  }

  public static TextComponent mapToTextComponent(
    int position,
    MapInfo mapInfo
  ) {
    ...
    message.setClickEvent(
      new ClickEvent(
        ...
        "/setnext " + mapInfo.getName()
      )
    );
    message.setHoverEvent(
      new HoverEvent(
        HoverEvent.Action.SHOW_TEXT,
        new ComponentBuilder(ChatColor.GOLD + mapInfo.getName())
          .append("\n\n")
          .append(ChatColor.GRAY + "Authors: ")
          .append(
            ChatColor.YELLOW +
            mapInfo
              .getAuthors()
          ...
          .append("\n")
          .append(ChatColor.GRAY + "Game Type: ")
          .append(ChatColor.YELLOW + mapInfo.getGametype().toString())
          .append("\n")
          .append(ChatColor.GRAY + "Version: ")
          .append(ChatColor.YELLOW + mapInfo.getVersion())
          .create()
      )
    );
    return message;
